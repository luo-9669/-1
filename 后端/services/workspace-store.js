...
function compactRestoredPageForOverview(page = {}) {
  ...
  return {
    ...
    html: '',
    files: [],
    coverImage: compactDataUrl(page.coverImage || ''),
    captureResult: null,
    previewUrl: page.previewUrl || (id ? `/api/workspace/restored-pages/${encodedId}/preview` : ''),
    frameUrl: page.frameUrl || (id ? `/api/workspace/restored-pages/${encodedId}/frame` : ''),
    sourceUrlPath: page.sourceUrlPath || (id ? `/api/workspace/restored-pages/${encodedId}/source` : ''),
    downloadUrl: page.downloadUrl || (id ? `/api/workspace/restored-pages/${encodedId}/download` : '')
  }
}
...
function getSettingValue(store, key = '') {
  const setting = store.settings.find((item) => item.key === key)
...
export function modelSettingsView(store) {
  ...
  return {
    provider: raw.provider || 'deterministic',
    baseUrl: raw.baseUrl || '',
    defaultModel: raw.defaultModel || 'gpt-5.5',
    apiSurface: raw.apiSurface || 'responses',
    timeoutMs: Number(raw.timeoutMs || 20000),
...
export async function saveModelSettings(store, payload = {}) {
  ...
  const enabled = payload.enabled !== undefined
    ...
    : (provider === 'openai-compatible' ? Boolean(apiKey || current.enabled) : Boolean(current.enabled))
  const value = {
    provider,
    apiKey,
    baseUrl: payload.baseUrl || current.baseUrl || '',
    defaultModel: payload.defaultModel || current.defaultModel || 'gpt-5.5',
    apiSurface: payload.apiSurface || current.apiSurface || 'responses',
    timeoutMs: Number(payload.timeoutMs || current.timeoutMs || 20000),
...
export function getRestoredPage(store, id = '') {
  const restoredPage = store.restoredPages.find((item) => item.id === id)
  if (!restoredPage) {

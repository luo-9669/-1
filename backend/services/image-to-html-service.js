...
function imageVisionPrompt(payload = {}) {
  ...
  return [
    ...
    '5. 优先使用 flex、grid、minmax、max-width、min-width: 0、aspect-ratio、object-fit、gap 等稳定布局能力。',
    '6. 固定宽度内容要有 max-width 和 overflow 兜底；卡片、表格、工具栏、图片区域在窄屏下要自然换行或堆叠。',
    '7. 首屏不能因视口变化出现文字压扁、按钮挤出、图片遮挡、卡片重叠或横向不可控溢出。',
    '8. Tab、分类栏、胶囊筛选和横向菜单超出容器时必须横向滚动，不要换行或挤压文字；右侧使用渐隐遮罩 / fade mask 暗示还有内容，滚动条可隐藏但交互必须可用。',
    '9. 390px / H5 移动端如果出现侧边栏、视频入口或大量顶部按钮，参考饿了么 H5 和大厂 H5 页面：移动端顶部按钮要隐藏或收纳，重操作收纳到“更多”按钮、底部导航、悬浮入口或抽屉菜单。',
    '10. 侧边栏视频在 H5 中不要把桌面侧边栏硬塞成九宫格或大面积空白；主操作最多保留 1 个主按钮，其余入口用横向 chip、底部 tab、浮动工具按钮或抽屉二级菜单承接。',
    '11. 避免夸张错落排版：标题、头像、tab、表单、按钮和卡片必须沿 4px 网格、同轴或基线对齐，不要为了像截图而制造随机错位、漂浮、过大的上下落差。',
    '',
    '4px 基础网格与圆角梯度：',

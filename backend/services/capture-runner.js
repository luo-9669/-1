...
function shouldSkipFallback(error, payload = {}) {
  if (error?.code === 'CAPTURE_BROWSER_SESSION_MISSING') return true
  return normalizeAuthMode(payload.authMode) === 'browser' && error?.code === 'CAPTURE_BROWSER_SESSION_EXPIRED'
}
...
export function createCaptureRunner(options = {}) {
  ...
  async function makeCaptureResult(payload = {}) {
    ...
    try {
      ...
      if (fallbackCaptureTarget && !shouldSkipFallback(error, { ...payload, authMode })) {
        try {

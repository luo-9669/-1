import { buildBlueprintDemoHtml, buildProjectBlueprint } from '../../frontend/src/services/projectBlueprint.js'
import { getBuiltinWorkflows } from '../../frontend/src/services/workflows.js'
import { runSkillGeneration, runSkillGenerationSync } from './generation-runner.js'
import { AUTO_SKILL_ID, SMART_RECOMMENDATION_SKILL_ID, orchestrateRequirementSkill, skillLabel } from './skill-orchestrator.js'
...
export function parseUploadedDocument(document = {}, index = 0) {
  ...
  if (!text) {
    const analyzableBinaryTypes = new Set(['image', 'pdf', 'doc'])
    if (analyzableBinaryTypes.has(kind)) {
      const typeLabel = kind === 'image' ? '图片' : kind === 'pdf' ? 'PDF' : 'Word'
      return {
        id: document.id || `upload-${index + 1}`,
        name,
        type: kind,
        status: 'parsed',
        text: `${typeLabel}素材已上传：${name}。当前分析需结合文件名、上传意图和前端补充说明识别需求；如需完整正文，请补充 DOCX/MD/TXT 或粘贴内容。`,
        summary: `${typeLabel} 素材：${name}`,
        recoveryActions: []
      }
    }
    return {
  ...
  return {
    ...
    selectedKnowledgeScope: document.selectedKnowledgeScope || null,
    knowledgeMaterialId: document.knowledgeMaterialId || '',
    status: 'parsed',
    text,
...
function workflowMeta(payload = {}, routing = orchestrateRequirementSkill(payload)) {
  ...
  const totalFlowSkillIds = ['total-design-flow', 'advanced-ux-requirement-analysis']
  const requestedTotalFlowSkillId = [payload.requestedSkillId, payload.skillId, routing.requestedSkillId]
    .find((value) => totalFlowSkillIds.includes(value))
  const isTotalDesignFlow = Boolean(requestedTotalFlowSkillId)
  const requestedSkillId = isTotalDesignFlow ? requestedTotalFlowSkillId : routing.requestedSkillId
  const resolvedSkillId = isTotalDesignFlow ? requestedTotalFlowSkillId : routing.resolvedSkillId
  const displayName = requestedTotalFlowSkillId === 'advanced-ux-requirement-analysis' ? '高级 UX 需求分析' : '总流程'
  return {
    demandScope: payload.demandScope === 'non-project' ? 'non-project' : 'project',
    requestedSkillId,
    resolvedSkillId,
    skillSelectionMode: routing.skillSelectionMode || (requestedSkillId === 'auto' || totalFlowSkillIds.includes(requestedSkillId) ? 'auto' : 'manual'),
    manualSkillSelected,
    displaySkillName: isTotalDesignFlow ? displayName : routing.displaySkillName || routing.resolvedSkillName || skillLabel(resolvedSkillId),
    detectedIntent: routing.detectedIntent,
    routingReason: routing.routingReason,
    requestedSkillName: routing.requestedSkillName,
    resolvedSkillName: isTotalDesignFlow ? displayName : routing.resolvedSkillName,
    skillId: resolvedSkillId,
    skillName: isTotalDesignFlow ? displayName : routing.resolvedSkillName
  }
}

function isSelectedKnowledgeScopeDocument(document = {}) {
  return document?.sourceType === 'selected-knowledge-scope' ||
    /^框选功能/.test(String(document?.name || '')) ||
    Boolean(document?.selectedKnowledgeScope)
}

function isScopedHomePromptShortcutDemand(input = '', documents = []) {
  const userText = String(input || '')
  if (!(/首页/.test(userText) && /(快捷|提示词|prompt|chip|输入)/i.test(userText))) return false
  const contextText = [
    userText,
    ...(documents || []).map((document) => `${document?.name || ''}\n${document?.text || document?.summary || ''}`)
  ].join('\n\n')
  return (documents || []).some(isSelectedKnowledgeScopeDocument) ||
    /\bjogg\b|podcast|播客|AI Podcast Generator|Topic\/input composer/i.test(contextText)
}

function homepagePromptShortcutBlueprint(blueprint = {}, input = '') {
  return {
    ...blueprint,
    intent: 'requirement-analysis',
    type: 'analysis-blueprint',
    title: '首页快捷提示词方案',
    profile: {
      ...(blueprint.profile || {}),
      productName: '首页快捷提示词',
      positioning: '首页输入区的轻量增强能力',
      primaryGoal: '在首页输入区增加快捷提示词，降低首次输入成本，并保持原有生成、登录拦截和工作区跳转链路。',
      sourceSummary: input || blueprint.profile?.sourceSummary || ''
    },
    framework: {
      ...(blueprint.framework || {}),
      title: '首页快捷提示词'
...
function interactionCanvasSourceText(blueprint = {}) {
  return [
    blueprint.title,
    blueprint.profile?.productName,
    blueprint.profile?.primaryGoal,
    blueprint.profile?.sourceSummary,
    ...(Array.isArray(blueprint.profile?.coreScenarios) ? blueprint.profile.coreScenarios : [])
  ].filter(Boolean).join(' ')
}

function isTeaOrderingInteraction(blueprint = {}) {
  const text = interactionCanvasSourceText(blueprint)
  return /茶饮|奶茶|咖啡|点单|取餐|菜单|商品定制|购物车|支付|会员|优惠|小程序/.test(text) &&
    /点单|取餐|菜单|购物车|支付|小程序/.test(text)
}

function teaOrderingInteractionPages() {
  const pages = [
    {
      title: '首页与选店',
      goal: '让用户进入小程序后快速定位门店，明确当前服务范围、营业状态和可下单入口。',
      modules: ['定位/城市切换', '附近门店列表', '门店营业状态', '门店公告', '开始点单按钮'],
      actions: ['授权定位', '切换城市', '搜索门店', '选择门店', '点击开始点单'],
      jumps: ['选择门店 -> 取餐方式', '定位失败 -> 手动选店', '门店休息 -> 更换门店'],
      states: ['加载：定位与门店列表骨架屏', '空状态：附近无门店，展示手动搜索', '失败：定位失败允许手动选择', '权限：未授权定位时说明用途'],
      dependencies: ['GET /api/stores/nearby', 'GET /api/stores/{id}/status', '用户定位权限', '门店营业时间与库存状态'],
      acceptance: ['未授权定位也能手动选店', '休息门店不能进入下单', '门店切换后清空不兼容购物车']
    },
    {
      title: '取餐方式',
      goal: '确认堂食、自提或外送方式，并让后续菜单、价格、库存和配送规则按取餐方式变化。',
      modules: ['取餐方式切换', '预计取餐/送达时间', '地址选择', '门店距离提示', '确认方式按钮'],
      actions: ['选择自提', '选择外送', '选择/新增地址', '确认取餐时间', '返回选店'],
      jumps: ['确认取餐方式 -> 菜单与商品', '外送地址不可达 -> 地址选择/更换门店', '返回 -> 首页与选店'],
      states: ['加载：配送范围校验中', '空状态：暂无可用地址', '失败：配送范围不可达', '权限：未登录时新增地址需登录'],
      dependencies: ['POST /api/order/context', 'GET /api/addresses', 'GET /api/delivery/range', '门店配送范围与自提时间规则'],
      acceptance: ['不同取餐方式影响菜单价格和可售商品', '外送不可达时不能继续结算', '取餐方式变更有购物车影响提示']
    },
    {
      title: '菜单与商品',
      goal: '帮助用户浏览分类、搜索商品、查看库存与活动，并进入商品定制或直接加入购物车。',
      modules: ['分类导航', '商品列表', '搜索框', '商品标签/库存', '购物车浮层入口'],
      actions: ['切换分类', '搜索商品', '点击商品', '快速加购', '打开购物车'],
      jumps: ['点击商品 -> 商品定制', '打开购物车 -> 购物车与结算', '商品售罄 -> 留在菜单并提示'],
      states: ['加载：分类与商品骨架屏', '空状态：分类无商品/搜索无结果', '失败：菜单加载失败可重试', '权限：会员价需登录展示'],
      dependencies: ['GET /api/menu/categories', 'GET /api/menu/products', 'GET /api/promotions/active', '门店库存、价格、会员标签'],
      acceptance: ['切换分类不丢购物车', '售罄商品不可加购', '搜索结果能返回菜单原位置']
    },
    {
      title: '商品定制',
      goal: '让用户选择规格、温度、糖度、加料和数量，并实时校验库存、价格和必选项。',
      modules: ['商品详情', '规格选择', '糖冰温度', '加料列表', '数量步进器', '加入购物车按钮'],
      actions: ['选择规格', '选择糖冰', '增减加料', '修改数量', '加入购物车'],
      jumps: ['加入购物车 -> 菜单与商品', '立即结算 -> 购物车与结算', '关闭 -> 菜单与商品'],
      states: ['加载：商品详情加载中', '空状态：暂无可选规格', '失败：价格计算失败允许重试', '权限：会员专属规格需登录'],
      dependencies: ['GET /api/products/{id}', 'POST /api/cart/items/validate', '规格库存、加料库存、价格计算规则'],
      acceptance: ['必选项未选不能加购', '价格随定制项实时变化', '库存不足时指出具体规格或加料']
    },
    {
      title: '购物车与结算',
      goal: '集中展示已选商品、优惠、取餐信息和费用明细，并引导用户确认后提交订单。',
      modules: ['购物车商品列表', '取餐方式摘要', '优惠券/会员权益', '费用明细', '备注', '去支付按钮'],
      actions: ['修改商品数量', '删除商品', '选择优惠券', '填写备注', '点击去支付'],
      jumps: ['去支付 -> 支付与订单生成', '编辑商品 -> 商品定制', '继续加购 -> 菜单与商品'],
      states: ['加载：价格重算中', '空状态：购物车为空', '失败：库存/价格变化需用户确认', '权限：使用优惠券需登录'],
      dependencies: ['GET /api/cart', 'POST /api/cart/reprice', 'GET /api/coupons/available', 'POST /api/orders/preview'],
      acceptance: ['结算前必须重新校验库存和价格', '优惠不可用要给原因', '购物车为空时主按钮回到菜单']
    },
    {
      title: '支付与订单生成',
      goal: '完成订单创建、支付调起和支付结果确认，保证失败、取消和重复支付可恢复。',
      modules: ['订单确认摘要', '支付方式', '支付倒计时', '支付状态反馈', '重新支付入口'],
      actions: ['提交订单', '选择支付方式', '确认支付', '取消支付', '重新支付'],
      jumps: ['支付成功 -> 制作与取餐通知', '支付失败 -> 停留支付页', '取消支付 -> 订单待支付'],
      states: ['加载：创建订单/调起支付中', '空状态：订单信息失效', '失败：支付失败可重试', '权限：未登录需登录后下单'],
      dependencies: ['POST /api/orders', 'POST /api/payments', 'GET /api/payments/{id}/status', '支付渠道 SDK 与订单幂等键'],
      acceptance: ['重复点击不会创建重复订单', '支付失败保留订单并可重试', '支付成功后订单状态准确同步']
    },
    {
      title: '制作与取餐通知',
      goal: '让用户看到订单制作进度、取餐码/配送状态和通知消息，减少到店或等待的不确定性。',
      modules: ['订单状态时间线', '取餐码', '预计完成时间', '门店信息', '通知订阅', '联系客服'],
      actions: ['查看订单进度', '订阅通知', '展示取餐码', '联系门店', '再来一单'],
      jumps: ['再来一单 -> 菜单与商品', '联系门店 -> 客服/电话', '查看详情 -> 订单详情'],
      states: ['加载：订单状态同步中', '空状态：暂无有效订单', '失败：状态刷新失败', '权限：订阅通知需用户授权'],
      dependencies: ['GET /api/orders/{id}', 'GET /api/orders/{id}/timeline', 'WebSocket/SSE 订单状态', '小程序订阅消息权限'],
      acceptance: ['订单状态变化能及时刷新', '取餐码离线也能查看最近一次结果', '通知授权失败不阻断查看订单']
    },
    {
      title: '会员与优惠',
      goal: '展示会员权益、积分、优惠券和活动，让用户在下单前后理解可用权益与使用限制。',
      modules: ['会员等级', '积分余额', '优惠券列表', '活动入口', '权益说明', '登录/开通按钮'],
      actions: ['登录会员', '领取优惠券', '查看权益', '使用优惠券', '查看积分明细'],
      jumps: ['使用优惠券 -> 购物车与结算', '登录成功 -> 会员与优惠', '活动商品 -> 菜单与商品'],
      states: ['加载：权益数据加载中', '空状态：暂无优惠券', '失败：领取失败可重试', '权限：未登录展示登录引导'],
      dependencies: ['GET /api/member/profile', 'GET /api/coupons', 'POST /api/coupons/{id}/claim', '积分与会员等级规则'],
      acceptance: ['未登录能看权益说明但不能领券', '不可用优惠券展示原因', '领券后结算页能同步可用优惠']
    },
    {
      title: '前后端与验收',
      goal: '把页面交互、接口、数据依赖和测试验收汇总成研发可执行的交接清单。',
      modules: ['页面接口清单', '状态码与错误码', '埋点事件', '测试用例', '上线验收清单'],
      actions: ['查看接口契约', '导出验收清单', '补充错误码', '确认上线范围'],
      jumps: ['补充页面问题 -> 对应页面节点', '确认完成 -> 交付/保存资产'],
      states: ['加载：接口清单生成中', '空状态：暂无接口定义', '失败：导出失败可重试', '权限：仅项目成员可导出'],
      dependencies: ['订单、门店、菜单、购物车、支付、会员全部 API', '埋点 SDK', '错误码字典', '测试环境与 Mock 数据'],
      acceptance: ['每个页面都有接口和异常状态', '主路径可从选店走到支付成功', '库存/支付/权限/网络失败都有测试用例']
    }
  ]
  return pages.map((page, index) => ({
    ...
    screen: {
      ...
      actions: page.actions.map((label, actionIndex) => ({
        label,
        to: page.jumps[actionIndex]?.split('->')[1]?.trim() || page.jumps[0]?.split('->')[1]?.trim() || '下一页面'
      })),
      wireframe: {
        components: page.modules.map((label) => ({ label, type: '模块' })),
        layout: page.modules
      }
    },
    spec: {
      ...
      states: page.states.map((item) => {
        ...
        return { name: name || '状态', description: description || item }
      }),
      exceptions: page.states
        .filter((item) => /失败|权限|空状态/.test(item))
        .map((item) => ({ trigger: item.split('：')[0], recovery: item.split('：')[1] || item }))
    },
    diagram: {
      summary: page.goal,
      layoutLines: page.modules
    },
    interaction: page
  }))
}

function extractInteractionPageNodes(blueprint = {}) {
  if (isTeaOrderingInteraction(blueprint)) return teaOrderingInteractionPages()
  const screens = Array.isArray(blueprint.demoScreens) ? blueprint.demoScreens : []
  const pageSpecs = Array.isArray(blueprint.interactionSpec?.pageSpecs) ? blueprint.interactionSpec.pageSpecs : []
  const diagramPages = Array.isArray(blueprint.frameworkDiagrams?.pages) ? blueprint.frameworkDiagrams.pages : []
  const pageTitles = new Set()
  const pages = []
  const addPage = (page = {}, index = pages.length) => {
    const title = String(page.title || page.pageName || page.name || `页面 ${index + 1}`).trim()
    if (!title || pageTitles.has(title)) return
    pageTitles.add(title)
    const screen = screens.find((item) => item.id === page.screenId || item.title === title) || {}
    const spec = pageSpecs.find((item) => item.screenId === page.screenId || item.pageName === title) || {}
    const diagram = diagramPages.find((item) => item.title === title) || {}
    pages.push({ title, screen, spec, diagram })
  }
  screens.forEach((screen, index) => addPage(screen, index))
  pageSpecs.forEach((page, index) => addPage(page, index))
  diagramPages.forEach((page, index) => addPage(page, index))
  if (!pages.length) {
    pages.push({
      title: blueprint.profile?.productName ? `${blueprint.profile.productName}首页` : '方案总览页',
      screen: {
        description: blueprint.profile?.sourceSummary || blueprint.profile?.primaryGoal || '承载本次交互方案的入口和核心信息。'
      },
      spec: {},
      diagram: {}
    })
...
function projectInteractionCanvasPlan(blueprint = {}, analysis = {}) {
  ...
  const pages = extractInteractionPageNodes(blueprint)
  const nodes = pages.map((page, index) => {
    ...
    const actions = compactList(
      interaction.actions || (screen.actions || []).map((action) => action.label || '操作'),
      ['补充用户点击动作']
    )
    const jumps = compactList(
      interaction.jumps || (screen.actions || []).map((action) => `${action.label || '操作'} -> ${action.to || '待定'}`),
      ['流程末端或待补充跳转']
    )
    const components = compactList(
      interaction.modules || (screen.wireframe?.components || []).map((item) => `${item.label || item.name}/${item.type || '组件'}`),
      ['核心信息区', '主操作区', '反馈状态区']
    )
    const states = compactList(
      interaction.states || (spec.states || []).map((state) => `${state.name || '状态'}：${state.description || state.rule || '待补充'}`),
      ['初始态', '加载态', '成功态', '空状态', '错误态']
    )
    const exceptions = compactList(
      (spec.exceptions || []).map((item) => `${item.trigger || '异常'}：${item.recovery || item.solution || '保留输入并允许重试'}`),
      ['网络失败：保留输入并允许重试', '权限不足：说明原因并给出下一步']
    )
    ...
    const acceptance = compactList(
      interaction.acceptance || [
        `${page.title} 主路径可完成`,
        '加载、空状态、失败、权限状态都有明确反馈',
        '前后端字段、错误码和验收用例可追踪'
      ],
      ['验收点待补充。']
    )
    const node = canvasNode(`interaction-page-${index + 1}`, page.title, `${projectName} 的「${page.title}」页面方案。`, [
      `页面目标：${screen.description || spec.goal || diagram.summary || `${page.title} 承接 ${productName} 的核心任务。`}`,
      `核心模块：${components.slice(0, 4).join('、')}`,
      `主要跳转：${jumps.slice(0, 3).join('、')}`,
      `状态覆盖：${states.slice(0, 4).join('、')}`
    ], index, ['补页面细节', '补状态异常', '补跳转关系'], [
      detailSection('页面目标', [
        screen.description || spec.goal || diagram.summary || `${page.title} 承接 ${productName} 的核心任务。`,
        `来源 Skill：${skillName}`
      ]),
      detailSection('页面核心模块', components),
      detailSection('用户点击动作', actions),
      detailSection('跳转到哪个页面', jumps),
      detailSection('加载 / 空状态 / 失败 / 权限等状态', [...states, ...exceptions].slice(0, 10)),
      detailSection('前后端接口或数据依赖', dependencies),
      detailSection('验收点', acceptance),
      detailSection('交付备注', [
        `项目：${projectName}`,
        `目标用户：${blueprint.profile?.targetUsers || '待确认'}`,
        `页面顺序：${index + 1}/${pages.length}`
      ])
    ])
    return {
      ...node,
      mode: 'interaction-page',
      type: 'page',
      groupName: '页面画布',
      width: 340,
      height: 230,
      agentScope: `只围绕「${page.title}」这个页面补充目标、模块、状态和跳转。`,
      pageMeta: {
        pageIndex: index + 1,
...
function normalizeGeneratedNodeId(value = '', index = 0) {
  ...
  const normalized = raw
    ...
    .replace(/^-|-$/g, '')
    .toLowerCase()
  return normalized || `model-node-${index + 1}`
}

function generatedCanvasNode(raw = {}, index = 0) {
  const title = String(raw.title || raw.name || raw.label || raw.nodeName || raw.heading || `画布节点 ${index + 1}`).trim()
  const content = compactList(
    Array.isArray(raw.content)
      ? raw.content
      : raw.content || raw.items || raw.detail || raw.highlights || raw.nodeContent || raw.summary,
    ['模型已生成该节点，等待继续补充细节。']
  )
  const quickActions = compactList(raw.quickActions || raw.actions || ['补充细节', '重新生成'], ['补充细节', '重新生成']).slice(0, 4)
  const rawSections = Array.isArray(raw.detailSections)
    ...
    : Array.isArray(raw.sections)
      ...
      : Array.isArray(raw.children)
        ? raw.children
        : []
  const detailSections = rawSections.length
    ? rawSections.map((section) => detailSection(section.title || section.heading || '模型生成内容', section.items || section.content || [], section.meta || ''))
    : [
        detailSection('重点内容', content)
      ]
  const node = canvasNode(
    normalizeGeneratedNodeId(raw.id || raw.key || raw.nodeId || raw.title || raw.nodeName || raw.heading, index),
    title,
    String(raw.summary || content[0] || '模型生成画布节点。').trim(),
    content,
    index,
    quickActions,
    detailSections
  ...
  return {
    ...node,
    type: raw.type || raw.nodeType || 'module',
    groupId: raw.groupId || '',
    groupName: raw.groupName || '',
    highlights: compactList(raw.highlights || content.slice(0, 3), content).slice(0, 4)
  }
}

function normalizeGeneratedEdges(rawEdges = [], nodes = []) {
  const nodeIds = new Set(nodes.map((node) => node.id))
  const edges = (Array.isArray(rawEdges) ? rawEdges : [])
    .map((edge, index) => {
      const from = normalizeGeneratedNodeId(edge.from || edge.source || edge.start || edge.fromNodeId || '')
      const to = normalizeGeneratedNodeId(edge.to || edge.target || edge.end || edge.toNodeId || '')
      if (!nodeIds.has(from) || !nodeIds.has(to) || from === to) return null
      return {
        id: String(edge.id || `${from}-${to}-${index + 1}`),
        from,
        to,
        label: String(edge.label || edge.title || edge.edgeLabel || '下一环节')
      }
    })
    .filter(Boolean)
  return edges.length
...
function groupGeneratedCanvasSource(output = {}) {
  const groups = Array.isArray(output.groups) ? output.groups : []
  const normalizedGroups = groups.map((group, groupIndex) => ({
    id: normalizeGeneratedNodeId(group.groupId || group.id || group.groupName || group.name, groupIndex),
    title: String(group.groupName || group.name || group.title || `分组 ${groupIndex + 1}`),
    summary: String(group.groupDesc || group.summary || group.description || '')
  }))
  const nodes = groups.flatMap((group, groupIndex) => {
    const groupMeta = normalizedGroups[groupIndex]
    return (Array.isArray(group.nodes) ? group.nodes : []).map((node) => ({
      ...node,
      groupId: groupMeta.id,
      groupName: groupMeta.title
    }))
  })
  return { groups: normalizedGroups, nodes }
}

function outlineGeneratedCanvasSource(output = {}) {
  const outline = Array.isArray(output.outline) ? output.outline : []
  return {
    groups: [],
    nodes: outline.map((item, index) => ({
      id: item.id || item.key || item.heading,
      title: item.heading || item.title,
      summary: item.summary,
      highlights: item.highlights || (item.children || []).map((child) => child.heading || child.title).filter(Boolean).slice(0, 3),
      children: item.children || [],
      nodeType: item.type || 'module',
      index
    }))
  }
}

function readableSummaryItems(readableSummary = {}) {
  const items = []
  if (readableSummary.oneSentence) items.push(`一句话结论：${readableSummary.oneSentence}`)
  if (readableSummary.userGoal) items.push(`用户目标：${readableSummary.userGoal}`)
  const coreModules = Array.isArray(readableSummary.coreModules) ? readableSummary.coreModules.filter(Boolean) : []
  if (coreModules.length) items.push(`核心模块：${coreModules.join('、')}`)
  const recommendedFlow = Array.isArray(readableSummary.recommendedFlow) ? readableSummary.recommendedFlow.filter(Boolean) : []
  if (recommendedFlow.length) items.push(`推荐流程：${recommendedFlow.map((item, index) => `${index + 1}. ${item}`).join('；')}`)
  const questions = Array.isArray(readableSummary.questions) ? readableSummary.questions.filter(Boolean) : []
  if (questions.length) items.push(`待确认：${questions.join('；')}`)
  return compactList(items, ['模型已完成可读总览，确认理解后可继续查看画布节点。'])
}

function readableSummaryPayload(output = {}) {
  const modelSummary = output.readableSummary && typeof output.readableSummary === 'object' ? output.readableSummary : null
  const fallbackModules = (Array.isArray(output.groups) ? output.groups : [])
    .map((group) => group.groupName || group.name || group.title)
    .filter(Boolean)
    .slice(0, 5)
  const fallbackQuestions = (output.qualityReport?.checks || [])
    .map((check) => check.label || check.detail || check.id)
    .filter(Boolean)
    .slice(0, 3)
  const readableSummary = modelSummary || {
    oneSentence: output.summary || output.canvasTitle || output.title || 'AI 已完成需求理解，建议先确认方向再查看画布。',
    userGoal: output.summary || '围绕当前输入生成可评审的结构画布。',
    coreModules: fallbackModules,
    recommendedFlow: ['确认 AI 理解', '查看核心模块', '展开节点详情', '沉淀需求文档'],
    questions: fallbackQuestions
  }
  const coreModules = Array.isArray(readableSummary.coreModules) ? readableSummary.coreModules.filter(Boolean).slice(0, 5) : []
  const recommendedFlow = Array.isArray(readableSummary.recommendedFlow) ? readableSummary.recommendedFlow.filter(Boolean).slice(0, 6) : []
  const questions = Array.isArray(readableSummary.questions) ? readableSummary.questions.filter(Boolean).slice(0, 4) : []
  return {
    title: 'AI 理解',
    summary: readableSummary.oneSentence || output.summary || '先确认 AI 对需求的理解是否正确，再查看结构画布。',
    userGoal: readableSummary.userGoal || readableSummary.oneSentence || output.summary || '',
    coreModules,
    recommendedFlow,
    questions,
    items: readableSummaryItems(readableSummary),
    detailSections: [
      detailSection('AI 理解的目标', [readableSummary.userGoal || readableSummary.oneSentence || output.summary || '待补充']),
      detailSection('核心模块', coreModules),
      detailSection('推荐推进流程', recommendedFlow),
      detailSection('待确认问题', questions.length ? questions : ['暂无明显待确认问题'])
    ]
  }
}

function modelGeneratedCanvasPlan(output = {}) {
  const grouped = groupGeneratedCanvasSource(output)
  const outlined = grouped.nodes.length ? grouped : outlineGeneratedCanvasSource(output)
  const rawNodes = outlined.nodes.length
    ? outlined.nodes
    : Array.isArray(output.canvas?.nodes) ? output.canvas.nodes : []
  const sourceNodes = rawNodes.length
    ? rawNodes
    : [
        {
          id: 'analysis',
          title: output.canvasTitle || output.title || '模型自由画布',
          summary: '模型已按输入和上传文档生成自由画布。',
          content: (output.sections || []).flatMap((section) => section.items || section.title || [])
        }
      ]
  const generatedNodes = sourceNodes.map(generatedCanvasNode)
  const nodes = generatedNodes.map((node, index) => ({
    ...node,
    x: 80 + index * 360,
    y: index % 2 === 0 ? 120 : 360
  }))
  const orderedTabs = Array.isArray(output.canvas?.orderedTabs) && output.canvas.orderedTabs.length
    ? output.canvas.orderedTabs.map((tab, index) => {
        const node = nodes[index] || nodes.find((item) => item.id === normalizeGeneratedNodeId(tab.key || tab.id || ''))
        return {
          key: node?.id || normalizeGeneratedNodeId(tab.key || tab.id || tab.label, index),
          label: String(tab.label || tab.title || node?.title || `节点 ${index + 1}`)
        }
      })
    : nodes.map((node) => ({ key: node.id, label: node.title }))
  const rawEdges = normalizeGeneratedEdges(output.edges || output.canvas?.edges || [], nodes)
  return {
    title: output.canvasTitle || output.title || '',
    summary: output.summary || '',
    canvasType: output.canvasType || '',
    layoutRule: output.layoutRule || 'top-down',
...
function applyGeneratedCanvasToAnalysis(analysis = {}) {
  if (analysis.resolvedSkillId !== 'none') return analysis
  if (!analysis.generation?.validation?.ok) return analysis
  const output = analysis.generation.output || {}
  const hasDrawableCanvas = Array.isArray(output.canvas?.nodes) || Array.isArray(output.groups) || Array.isArray(output.outline)
  if (!hasDrawableCanvas) return analysis
  const outputTitle = output.canvasTitle || output.title || '不选择 Skill 自由画布方案'
  analysis.aiSummary = readableSummaryPayload(output)
  analysis.canvas = modelGeneratedCanvasPlan(analysis.generation.output)
  analysis.blueprint = {
    ...(analysis.blueprint || {}),
    type: 'model-generated-canvas',
    title: outputTitle,
    skillId: 'none',
    resolvedSkillId: 'none',
    displaySkillName: '不选择 Skill'
  }
  if (analysis.generation.output.designMarkdown) {
    analysis.designMarkdown = analysis.generation.output.designMarkdown
    analysis.blueprint = {
      ...(analysis.blueprint || {}),
      designMarkdown: analysis.generation.output.designMarkdown
    }
  }
  return analysis
}

function applyNoneModelFailureToAnalysis(analysis = {}) {
  if (analysis.resolvedSkillId !== 'none' || analysis.generation?.status !== 'failed') return analysis
  const reason = analysis.generation.fallbackReason || '模型未返回可用自由画布。'
  ...
  analysis.blueprint = {
    type: 'model-generation-required',
    title: '等待模型生成自由画布',
    skillId: 'none',
    resolvedSkillId: 'none',
    displaySkillName: '不选择 Skill',
  ...
  analysis.nextActions = [
    { id: 'check-model-settings', label: '检查模型配置' },
    { id: 'reanalyze-documents', label: '重新分析文档' }
  ]
  return analysis
}
...
function canvasPlan(blueprint = {}, analysis = {}) {
  ...
  const usesSmartRecommendation = [analysis.skillId, analysis.resolvedSkillId, analysis.requestedSkillId].includes(SMART_RECOMMENDATION_SKILL_ID) ||
    analysis.requestedSkillId === AUTO_SKILL_ID ||
    analysis.skillSelectionMode === 'auto'
  ...
  if (analysis.demandScope === 'non-project' && !usesSmartRecommendation) return nonProjectCanvasPlan(blueprint, analysis)
  const specPages = blueprint.interactionSpec?.pageSpecs || []
...
export function analyzeRequirementDocuments(payload = {}, options = {}) {
  ...
  const protectedBlueprint = meta.detectedIntent === 'requirement-analysis' && isScopedHomePromptShortcutDemand(payload.input || '', parsed.documents)
    ? homepagePromptShortcutBlueprint(baseBlueprint, payload.input || '')
    : baseBlueprint
  const routedBlueprint = meta.detectedIntent === 'auth-modal' ? toAuthModalBlueprint(protectedBlueprint, input) : protectedBlueprint
  const blueprint = {
    ...routedBlueprint,
  ...
  const shouldRunGeneration = ['auth-page-generation', 'smart-recommendation-skill', 'total-design-flow', 'advanced-ux-requirement-analysis'].includes(meta.resolvedSkillId)
  ...
  const analysis = {
    ...
    skillName: meta.skillName,
    input: payload.input || '',
    totalDesignFlowMode: payload.totalDesignFlowMode || '',
    project,
...
export async function analyzeRequirementDocumentsWithGeneration(payload = {}, options = {}) {
  ...
  if ((!provider || provider.name === 'deterministic') && analysis.resolvedSkillId === 'none') {
    analysis.status = 'failed'
    analysis.generation = {
      status: 'failed',
      provider: '',
      model: '',
      fallbackUsed: false,
      fallbackReason: '不选择 Skill 必须调用真实模型生成画布，当前未配置可用模型或仍在使用 deterministic fallback。',
      validation: { ok: false, schemaId: 'smart-canvas', missing: [], issues: ['model provider required for none skill'] },
      output: null,
  ...
  if (!provider || !['none', 'auth-page-generation', 'smart-recommendation-skill', 'total-design-flow', 'advanced-ux-requirement-analysis'].includes(analysis.resolvedSkillId)) return analysis
  ...
  analysis.generation = await runSkillGeneration({
    ...
    generationTimeoutMs: payload.generationTimeoutMs ?? payload.timeoutMs ?? options.generationTimeoutMs,
    onModelDelta: payload.onModelDelta || options.onModelDelta,
    onModelEvent: payload.onModelEvent || options.onModelEvent,

const SYSTEM_SKILLS = [
  ...
  },
  {
    ...
    promptTemplate: [
      ...
      '该 Skill 复制总流程运行链路，但需求分析第一阶段必须按照最新高级 UX 10 步论证链输出：原始需求分析、设计问题定义、用户与场景、假设与验证、设计机会、整体交互链路、三套设计方案、异常流补充、推荐方案建议、设计优先级与分阶段计划。',
      ...
      '该 Skill 的完整交付分三类产物：A. 高级 UX 需求分析 Markdown，用于导入 10 个需求分析画布节点；B. 页面交互框架与说明 Markdown，用于导入交互低保页面节点；C. Draw.io 主流程图、Draw.io 状态图和低保真线框图图片作为全局/页面产物。',
      ...
      '分析链路必须逐步传递：01 原始需求进入 02 设计问题，02 问题定义进入 03 用户与场景，03 场景假设进入 04 假设与验证，04 验证结论进入 05 设计机会，05 机会进入 06 整体交互链路，06 关键节点进入 07 三套设计方案，07 方案风险进入 08 异常流，08 异常约束进入 09 推荐方案建议，09 推荐结论进入 10 设计优先级与分阶段计划。',
      '页面交互框架与说明必须基于 10 节点结论继续生成，包含页面总览、页面流转总览、主流程、状态机、逐页页面定位/页面框架/文本布局图/交互规则/异常状态、交互规则表（自有产品）、全局交互规范和交付物清单。',
      '低保真线框图和 Draw.io 是后续真实产物：Draw.io 主流程图和状态图作为 Agent 文件产物展示，低保真图片绑定页面节点；没有真实图片或 .drawio/.xml 文件时，只能标注待生成或触发条件，不能声称已生成。',
      '输出 totalDesignFlow.requirementDissectionArtifact.productAnalysisPipeline.tabs 时，tabs 必须使用 10 个节点 id：ux-original-requirement-analysis、ux-design-problem-definition、ux-user-scenario、ux-assumption-validation、ux-design-opportunity、ux-interaction-chain、ux-three-design-solutions、ux-exception-flow、ux-recommendation-decision、ux-priority-phasing。',

function normalizeAuthMode(authMode = 'public') {
  return ['public', 'browser', 'cookie', 'saved-state'].includes(authMode) ? authMode : 'public'
}
...
export function compileCaptureTask(input = {}) {
  ...
  return {
    ...
    source: {
      url: analysis.normalizedUrl || String(payload.url || '').trim()
    },
    auth: {
      mode: authMode,
      sessionId: authMode === 'browser' ? payload.sessionId || '' : '',
      cookieText: authMode === 'cookie' ? payload.cookieText || '' : '',
      storageStateKey: authMode === 'saved-state' ? payload.storageStateKey || '' : ''
    },
    execution: {

...
const DEFAULT_UX_PAGE_INTERACTION_REFERENCE_EXAMPLE_PATH = fileURLToPath(new URL('../../docs/skills/advanced-ux/交互低保阶段二输出参考文档.md', import.meta.url))
...
export function uploadRoutes(options = {}) {
  ...
  const advancedUxDirectMarkdownPrompt = (payload = {}, frameworkMarkdown = '', outputStandardMarkdown = '', referenceExampleMarkdown = '', evidencePack = {}) => ({
    systemPrompt: [
      ...
      '- 步骤①必须包含新版扩展框架：需求冲突识别表、多分支5Whys追溯表、问题重写3版本表；需求冲突识别至少 2 条，多分支假设至少 3 条，问题重写必须使用 JTBD 标准句式。',
      ...
      '- 步骤②必须包含症状-原因-方案拆分表和功能边界对比表；症状-原因-方案拆分表至少 3 条路径，功能边界对比表至少 5 个维度，字段可按项目自然命名但必须覆盖症状/原因/方案、边界/差异/判定语义。',
      '- 步骤③“用户与场景”必须包含标准 Journey Map 表格，覆盖阶段、触点、用户行为、情绪曲线/情绪、痛点、机会点、证据/置信度；至少 4 个阶段。',
      '- 步骤③必须包含 JTBD 任务定义，至少 4 条任务，覆盖不同用户类型/场景/行为/结果/障碍/痛点/关键洞察。',
      ...
      '- 步骤④必须包含风险优先级矩阵和用户-场景-任务矩阵；风险优先级矩阵覆盖风险、概率/发生概率、影响、优先级、说明/验证方式；用户-场景-任务矩阵覆盖用户类型、关键场景、核心任务、当前障碍、期望能力。',
      ...
      '- 步骤⑤必须包含现有流程诊断（逐节点分析），用表格覆盖节点、用户行为/当前问题、可能卡点/影响、风险等级/优化方向，并提出至少 2 条新流程路径。',
      ...
      '- 步骤⑨必须包含设计方向对比表和推荐决策卡片；设计方向对比至少 3 个方向，推荐决策卡片必须包含推荐方案/方向、至少 3 条核心理由、关键假设、不确定性、下一步建议和置信度。',
  ...
  const advancedUxMarkdownCompletenessPrompt = (payload = {}, markdown = '', qualityIssues = [], evidencePack = {}) => ({
    systemPrompt: [
      ...
      '- 必须补齐新版阶段一扩展框架：需求冲突识别表、多分支5Whys追溯表、问题重写3版本表、症状-原因-方案拆分表、功能边界对比表、JTBD 任务定义、风险优先级矩阵、现有流程诊断、用户-场景-任务矩阵、设计方向对比表、推荐决策卡片。',
      ...
      '- 在“原始需求分析 / 需求冲突识别表”中，必须识别需求来源、定位、动机、输入输出或功能边界中的冲突，字段覆盖冲突描述/类型/影响/消解建议/置信度。',
      '- 在“原始需求分析 / 多分支5Whys”中，必须输出至少 3 条假设路径，并分别追问到 Why 2，再汇总根因假设、用户类型、核心诉求和产品定义差异。',
      '- 在“原始需求分析 / 问题重写3版本”中，必须使用 JTBD 标准句式输出 3 个版本，并标注对应根因方向、优先级和置信度。',
      ...
      '- 症状-原因-方案拆分表必须用 Markdown 表格输出，至少 3 条路径，语义覆盖症状描述、可能原因、方案方向、影响力/可行性、置信度。',
      '- 功能边界对比表必须用 Markdown 表格输出，至少 5 个维度，语义覆盖新需求定位、现有功能定位、差异说明和边界判定。',
      ...
      '- 在“设计机会 / 现有流程诊断”中，必须逐节点诊断当前流程卡点，字段覆盖节点、用户行为/当前问题、可能卡点/影响、风险等级/优化方向，并给出至少 2 条新流程路径。',
      ...
      '- 在“推荐方案建议 / 设计方向对比表”中，必须对比至少 3 个方向，覆盖核心思路、核心价值、风险/前提和与现有产品的关系，或以加权评分方式展示对比依据。',
      '- 在“推荐方案建议 / 六顶思考帽评审”中，必须用白帽、红帽、黑帽、黄帽、绿帽、蓝帽多维审视后再给推荐结论。',
      '- “推荐方案建议”章节内必须保留明确三级标题“### 2. 六顶思考帽评审”；如果上一轮只在“三套设计方案”或正文里提到六帽，必须移动或补写到这里。',
      '- 在“推荐方案建议 / 推荐决策卡片”中，必须输出推荐方案/方向、至少 3 条核心理由、方案融合点或排除理由、关键假设、不确定性、下一步建议和置信度。',
  ...
  const advancedUxMethodologyReviewIssues = (text = '', tables = []) => {
    ...
    const conflictRows = advancedUxRowsForAnyColumnGroup(tables, [
      ['冲突维度', '矛盾描述', '影响', '置信度'],
      ['冲突描述', '冲突类型', '涉及来源', '严重度', '消解建议', '置信度'],
      ['冲突描述', '冲突类型', '影响', '消解建议', '置信度']
    ])
    if (!conflictRows.length) {
      issues.push('缺少需求冲突识别表：需覆盖冲突描述/类型、影响、消解建议和置信度')
    } else if (conflictRows.length < 2) {
      issues.push('需求冲突识别表至少需要 2 条内部冲突')
    }
    const branchHypothesisRows = advancedUxRowsForAnyColumnGroup(tables, [
      ['假设路径', '假设描述', '对应根因方向'],
      ['根因假设', '用户类型', '核心诉求', '对"做什么"的定义'],
      ['根因假设', '用户类型', '核心诉求', '产品定义']
    ])
    const branchWhyRows = advancedUxRowsForAnyColumnGroup(tables, [
      ['假设', 'Why 2 追问', '可能原因', '推论'],
      ['假设路径', 'Why 1', 'Why 2', '推论'],
      ['假设路径', '追问', '可能原因', '推论']
    ])
    if (!/多分支/.test(text) || (!branchHypothesisRows.length && !branchWhyRows.length)) {
      issues.push('缺少多分支5Whys追溯表：需至少输出 3 条假设路径并追问到 Why 2')
    } else if (Math.max(branchHypothesisRows.length, branchWhyRows.length) < 3) {
      issues.push('多分支5Whys追溯表至少需要 3 条假设路径')
    }
    const problemRewriteRows = advancedUxRowsForAnyColumnGroup(tables, [
      ['版本编号', 'JTBD问题陈述', '对应根因方向', '优先级', '置信度'],
      ['版本', 'JTBD问题陈述', '对应根因方向', '优先级', '置信度'],
      ['版本编号', '问题陈述', '对应根因方向', '优先级', '置信度']
    ])
    const problemRewriteText = advancedUxSectionBody(text, '原始需求分析')
    const problemRewriteVersions = [...problemRewriteText.matchAll(/版本\s*[123一二三]/g)].length
    if (!/问题重写/.test(text) || (!problemRewriteRows.length && problemRewriteVersions < 3)) {
      issues.push('缺少问题重写3版本表：需用 JTBD 标准句式输出 3 个问题版本')
    } else if (problemRewriteRows.length > 0 && problemRewriteRows.length < 3) {
      issues.push('问题重写3版本表至少需要 3 个版本')
    }
    ...
    const symptomRows = advancedUxRowsForAnyColumnGroup(tables, [
      ['症状', '可能原因', '方案方向', '置信度'],
      ['症状描述', '可能原因', '方案方向', '影响力', '可行性', '置信度'],
      ['症状描述', '可能原因', '方案方向', '置信度']
    ])
    if (!symptomRows.length) {
      issues.push('缺少症状-原因-方案拆分表：需覆盖症状、可能原因、方案方向和置信度')
    } else if (symptomRows.length < 3) {
      issues.push('症状-原因-方案拆分表至少需要 3 条独立路径')
    }
    const boundaryRows = advancedUxRowsForAnyColumnGroup(tables, [
      ['维度', '现有功能A', '新功能B', '差异说明'],
      ['对比维度', '新需求定位', '现有功能定位', '差异说明', '边界判定'],
      ['维度', '现有功能', '新功能', '差异说明']
    ])
    if (!boundaryRows.length) {
      issues.push('缺少功能边界对比表：需覆盖新旧功能定位、差异说明和边界判定')
    } else if (boundaryRows.length < 5) {
      issues.push('功能边界对比表至少需要 5 个对比维度')
    }
    ...
    const jtbdRows = advancedUxRowsForAnyColumnGroup(tables, [
      ['JTBD 描述', '用户类型', '关键洞察', '优先级'],
      ['用户类型', '场景', '行为', '结果', '障碍', '痛点', '关键洞察'],
      ['用户类型', '关键场景', '行为', '期望结果', '当前障碍', '痛点']
    ])
    if (!/JTBD|Jobs To Be Done|任务定义/.test(text) || !jtbdRows.length) {
      issues.push('缺少JTBD 任务定义：需覆盖用户类型、场景、行为、结果、障碍、痛点和关键洞察')
    } else if (jtbdRows.length < 4) {
      issues.push('JTBD 任务定义至少需要 4 条核心任务')
    }
    ...
    const riskPriorityRows = advancedUxRowsForAnyColumnGroup(tables, [
      ['风险', '概率', '影响', '优先级'],
      ['风险', '发生概率', '用户影响', '优先级'],
      ['风险', '影响', '发生概率', '优先级']
    ])
    if (!riskPriorityRows.length) {
      issues.push('缺少风险优先级矩阵：需覆盖风险、概率/发生概率、影响和优先级')
    } else if (riskPriorityRows.length < 3) {
      issues.push('风险优先级矩阵至少需要 3 条关键风险')
    }
    const userScenarioTaskRows = advancedUxRowsForAnyColumnGroup(tables, [
      ['用户类型', '关键场景', '核心任务', '当前障碍', '期望能力'],
      ['用户类型', '关键场景', '核心任务', '当前障碍', '期望能力', '需求差异', '置信度']
    ])
    if (!userScenarioTaskRows.length) {
      issues.push('缺少用户-场景-任务矩阵：需覆盖用户类型、关键场景、核心任务、当前障碍和期望能力')
    } else if (userScenarioTaskRows.length < 3) {
      issues.push('用户-场景-任务矩阵至少需要 3 条用户场景任务记录')
    }
    if (!/假设回检/.test(text)) {
      issues.push('缺少假设回检')
    }
    ...
    const existingFlowDiagnosisRows = advancedUxRowsForAnyColumnGroup(tables, [
      ['节点', '用户行为', '可能的卡点', '风险等级'],
      ['节点', '当前问题', '影响', '优化方向'],
      ['节点', '用户行为', '可能卡点', '风险等级']
    ])
    if (!existingFlowDiagnosisRows.length) {
      issues.push('缺少现有流程诊断：需逐节点覆盖用户行为/当前问题、卡点/影响和风险等级/优化方向')
    } else if (existingFlowDiagnosisRows.length < 3) {
      issues.push('现有流程诊断至少需要 3 个流程节点')
    }
    ...
    const designDirectionRows = advancedUxRowsForAnyColumnGroup(tables, [
      ['方向', '核心思路', '核心价值', '风险/前提', '与现有产品的关系'],
      ['方向', '核心思路', '核心价值', '风险', '与现有产品的关系'],
      ['对比维度', '方向A评分', '方向B评分', '方向C评分', '权重']
    ])
    if (!designDirectionRows.length) {
      issues.push('缺少设计方向对比表：需至少对比 3 个设计方向或给出加权评分矩阵')
    } else if (designDirectionRows.length < 3) {
      issues.push('设计方向对比表至少需要 3 个方向/维度')
    }
    const recommendationDecisionRows = advancedUxRowsForAnyColumnGroup(tables, [
      ['推荐方案', '核心理由', '方案融合点', '后续演进方向', '置信度'],
      ['推荐方向', '核心理由', '关键假设', '不确定性', '置信度'],
      ['维度', '内容']
    ])
    const recommendationSection = advancedUxSectionBody(text, '推荐方案建议')
    const hasRecommendationDecisionCard = /推荐决策/.test(recommendationSection) &&
      (/推荐方向|推荐方案/.test(recommendationSection)) &&
      (/推荐原因|核心理由/.test(recommendationSection)) &&
      /置信度/.test(recommendationSection)
    if (!recommendationDecisionRows.length && !hasRecommendationDecisionCard) {
      issues.push('缺少推荐决策卡片：需包含推荐方案/方向、核心理由、关键假设、不确定性、下一步建议和置信度')
    }
  ...
  const pageInteractionLatestStageTwoIssues = (markdown = '', tables = []) => {
    ...
    if (!/低保真(?:原型)?完成度|低保真线框图规范/.test(text)) {
      issues.push('缺少低保真完成度规范：需说明按钮文案、placeholder、比例、交互标注和状态区分')
    }
    if (!/Draw\.io[\s\S]{0,120}(图规划|规划说明|产出规范)/i.test(text)) {
      issues.push('缺少 Draw.io 图规划说明：需覆盖图类型、节点列表、主路径、分支路径、标注信息和输出格式')
    }
    if (!/(全局)?自检清单|19项自检/.test(text)) {
      issues.push('缺少全局自检清单：需覆盖流程、页面、状态机、低保真、方案、编号联动和产物规范')
    }
  ...
  const pageInteractionDocumentQualityIssues = (markdown = '') => {
    ...
    ;['文档概述', '页面总览', '页面流转总览', '信息架构实体表', '核心用户流程', '状态机', '弹窗与抽屉定义表', '逐页交互说明', '关键断点与优化节点', '交互规则表', '全局交互规范', '页面3方案', '方案验证与收敛', '交付物清单', '产物规范说明'].forEach((heading) => {
      if (!new RegExp(`^##\\s+.*${heading}`, 'm').test(text)) issues.push(`缺少章节：${heading}`)
    })
  ...
  const pageInteractionDocumentPrompt = (payload = {}, report = {}, outputStandardMarkdown = '', pageInteractionSkillMarkdown = '', pageInteractionReferenceMarkdown = '', qualityIssues = []) => {
    ...
    return {
      ...
      '- 产物规范说明必须包含：低保真线框图规范、低保真原型完成度自检清单（也称低保真完成度自检清单）、Draw.io 图规划说明、Draw.io 产出规范、文档结构规范、方案对比规范、全局自检清单（19项）。',
      '- 低保真完成度自检清单必须覆盖：按钮有文案、输入框有 placeholder、图片区有比例标注、区域宽度比例清晰、可点击区域标注 IR 编号、不同状态有视觉区分、主按钮突出。',
      '- Draw.io 图规划说明必须在生成真实 Draw.io 文件前先输出，字段覆盖：图类型、包含节点、主路径、分支路径、需要标注的信息、输出格式；未生成真实 .drawio/.xml 文件时只能写“待生成/规划完成”，不能写“已生成”。',
      '- 全局自检清单必须覆盖至少 19 项检查，包含流程步骤、页面总览、核心页面状态机、核心页面低保真、异常流、方案对比、置信度、信息架构、编号联动、全局交互规范、关键断点、表格占比、弹窗定义、P-S Fit、埋点、待确认事项、低保真完成度、Draw.io 规划、产物规范说明。',
  ...
  const advancedUxMarkdownPrompt = (payload = {}, result = {}) => ({
    systemPrompt: [
      ...
      '原始需求分析必须额外包含新版扩展小节：需求冲突识别表、多分支5Whys追溯表、问题重写3版本表；这三个小节必须进入 Markdown 详情块，不能只写一句结论。',
      ...
      '设计机会必须包含现有流程诊断（逐节点分析），表头语义覆盖节点、用户行为/当前问题、可能卡点/影响、风险等级/优化方向，并给出至少 2 条新流程路径。',
      ...
      '推荐方案建议必须包含设计方向对比表和推荐决策卡片；设计方向对比至少 3 个方向，推荐决策卡片必须包含推荐方案/方向、至少 3 条核心理由、关键假设、不确定性、下一步建议和置信度。',

...
async function requestSse(baseUrl, path, options = {}) {
  ...
  const timeoutMs = options.timeoutMs === 0 ? 0 : (options.timeoutMs || 90000)
  const controller = new AbortController()
  let timedOut = false
  ...
  try {
    ...
    if (!done) {
      return {
        ok: false,
        status: 'interrupted',
        message: '流式请求未收到完成事件，正在尝试恢复最终结果',
        data: { interrupted: true, code: 'SSE_DONE_MISSING' }
      }
    }
...
export const api = {
  auth: {
    me(config) {
      return request(config.apiBaseUrl, '/api/auth/me')
    },
    login(config, payload) {
      return request(config.apiBaseUrl, '/api/auth/login', {
        method: 'POST',
        body: JSON.stringify(payload)
      })
    },
    register(config, payload) {
      return request(config.apiBaseUrl, '/api/auth/register', {
        method: 'POST',
        body: JSON.stringify(payload)
      })
    },
    logout(config) {
      return request(config.apiBaseUrl, '/api/auth/logout', {
        method: 'POST',
        body: JSON.stringify({})
      })
    }
  ...
  workspace: {
    ...
    deleteAsset(config, id) {
      return request(config.apiBaseUrl, `/api/workspace/assets/${encodeURIComponent(id)}`, {
        method: 'DELETE'
      })
    },
    ...
    startProjectRuntime(config, payload) {
      return request(config.apiBaseUrl, '/api/workspace/project-runtime/start', {
        method: 'POST',
        body: JSON.stringify(payload),
        timeoutMs: 240000
      })
    },
    stopProjectRuntime(config, payload) {
      return request(config.apiBaseUrl, '/api/workspace/project-runtime/stop', {
        method: 'POST',
        body: JSON.stringify(payload),
        timeoutMs: 30000
      })
    ...
    importWorkflowAcceptanceKnowledge(config, runId, payload = {}) {
      return request(config.apiBaseUrl, `/api/workspace/workflow-runs/${encodeURIComponent(runId)}/import-acceptance-knowledge`, {
        method: 'POST',
        body: JSON.stringify(payload),
        timeoutMs: 30000
      })
    ...
    deleteWorkflowRun(config, runId) {
      return request(config.apiBaseUrl, `/api/workspace/workflow-runs/${encodeURIComponent(runId)}`, {
        method: 'DELETE'
      })
    },
  ...
  uploads: {
    ...
    analyzeDocuments(config, payload, options = {}) {
      return request(config.apiBaseUrl, '/api/uploads/documents/analyze', {
        method: 'POST',
        body: JSON.stringify({ ...payload, timeoutMs: options.timeoutMs ?? payload.timeoutMs }),
    ...
    repairAdvancedUxAnalysis(config, payload = {}) {
      return request(config.apiBaseUrl, '/api/uploads/documents/analyze/advanced-ux/repair', {
        method: 'POST',
        body: JSON.stringify(payload),
        timeoutMs: 0
      })
    },
    analyzeStageStream(config, payload, options = {}) {
      return requestSse(config.apiBaseUrl, '/api/uploads/documents/analyze/stage/stream', {
        method: 'POST',
        body: JSON.stringify({ ...payload, timeoutMs: options.timeoutMs ?? payload.timeoutMs }),
        onEvent: options.onEvent,
        signal: options.signal,
  ...
  capture: {
    ...
    saveBrowserSessionState(config, payload) {
      return request(config.captureBaseUrl || config.apiBaseUrl, '/api/browser-sessions/save-state', {
        method: 'POST',
        body: JSON.stringify(payload),
        timeoutMs: 20000
      })
    },
    listBrowserSessionStates(config, payload = {}) {
      const params = new URLSearchParams()
      if (payload.projectId) params.set('projectId', payload.projectId)
      const suffix = params.toString() ? `?${params.toString()}` : ''
      return request(config.captureBaseUrl || config.apiBaseUrl, `/api/browser-sessions/states${suffix}`, {
        method: 'GET',
        timeoutMs: 15000
      })
    },
  ...
  knowledge: {
    ...
    fromProjectPackage(config, payload) {
      return request(config.knowledgeBaseUrl || config.apiBaseUrl, '/api/knowledge/from-project-package', {
        method: 'POST',
        body: JSON.stringify(payload),
        timeoutMs: 120000
      })
  ...
  workflows: {
    ...
    async appendMessageStream(config, runId, stepId, payload, options = {}) {
      const streamed = await requestSse(config.apiBaseUrl, `/api/workspace/workflow-runs/${encodeURIComponent(runId)}/messages/stream`, {
        method: 'POST',
        body: JSON.stringify({ ...payload, stepId, timeoutMs: options.timeoutMs ?? payload.timeoutMs }),
        onEvent: options.onEvent,
        signal: options.signal,
        timeoutMs: options.timeoutMs ?? 0
      })
    ...
    generateCanvasNodeArtifact(config, runId, nodeId, payload = {}, options = {}) {
      return request(config.apiBaseUrl, `/api/workspace/workflow-runs/${encodeURIComponent(runId)}/canvas-nodes/${encodeURIComponent(nodeId)}/generate-artifact`, {
        method: 'POST',
        body: JSON.stringify({ ...payload, timeoutMs: options.timeoutMs ?? payload.timeoutMs }),
        timeoutMs: options.timeoutMs ?? 0
      })
  ...
  competitors: {
    ...
    update(config, competitorId, payload = {}) {
      return request(config.competitorBaseUrl || config.apiBaseUrl, `/api/competitors/${encodeURIComponent(competitorId)}`, {
        method: 'PATCH',
        body: JSON.stringify(payload)
      })
    },
    delete(config, competitorId, payload = {}) {
      const params = new URLSearchParams()
      if (payload.projectId) params.set('projectId', payload.projectId)
      const suffix = params.toString() ? `?${params.toString()}` : ''
      return request(config.competitorBaseUrl || config.apiBaseUrl, `/api/competitors/${encodeURIComponent(competitorId)}${suffix}`, {
        method: 'DELETE'
      })
    }
  },
  competitorAnalysis: {
    listRecords(config, payload = {}) {
      const params = new URLSearchParams()
      if (payload.projectId) params.set('projectId', payload.projectId)
      if (payload.kind) params.set('kind', payload.kind)
      const suffix = params.toString() ? `?${params.toString()}` : ''
      return request(config.competitorBaseUrl || config.apiBaseUrl, `/api/competitor-analysis/records${suffix}`, {
        method: 'GET',
        timeoutMs: 15000
      })
    },
    createRecord(config, body = {}) {
      return request(config.competitorBaseUrl || config.apiBaseUrl, '/api/competitor-analysis/records', {
        method: 'POST',
        body: JSON.stringify(body),
        timeoutMs: 15000
      })
    },
    deleteRecord(config, payload = {}) {
      const projectId = payload.projectId || 'default'
      const recordId = payload.recordId || payload.id || ''
      const params = new URLSearchParams()
      params.set('projectId', projectId)
      return request(config.competitorBaseUrl || config.apiBaseUrl, `/api/competitor-analysis/records/${encodeURIComponent(recordId)}?${params.toString()}`, {
        method: 'DELETE',
        timeoutMs: 15000
      })
    },
    latest(config, payload = {}) {
      ...
      if (payload.kind) params.set('kind', payload.kind)
      const suffix = params.toString() ? `?${params.toString()}` : ''
      return request(config.competitorBaseUrl || config.apiBaseUrl, `/api/competitor-analysis/latest${suffix}`, {
        method: 'GET',
        timeoutMs: 15000
      })
    },
    run(config, body = {}) {
      return request(config.competitorBaseUrl || config.apiBaseUrl, '/api/competitor-analysis/run', {
        method: 'POST',
        body: JSON.stringify(body),
        timeoutMs: 0
      })

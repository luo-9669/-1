...
export function normalizeFactoryWorkspace(state = {}, options = {}) {
  const previousRestoredPageMap = new Map(
    (Array.isArray(options.previousRestoredPages) ? options.previousRestoredPages : [])
      .filter((page) => page?.id)
      .map((page) => [page.id, page])
  )
  const restoredPages = Array.isArray(state.restoredPages)
    ? state.restoredPages.map((page) => {
        const previousPage = previousRestoredPageMap.get(page.id) || null
        const pageHasHtml = Boolean(page.html)
          || (Array.isArray(page.files) && page.files.some((file) => file?.path === 'index.html' && file.content))
        const previousHasHtml = Boolean(previousPage?.html)
          || (Array.isArray(previousPage?.files) && previousPage.files.some((file) => file?.path === 'index.html' && file.content))
        const sourcePage = !pageHasHtml && previousHasHtml
          ? {
              ...page,
              html: previousPage.html,
              files: previousPage.files
            }
          : page
        const htmlFile = Array.isArray(sourcePage.files)
          ? sourcePage.files.find((file) => file.path === 'index.html' && file.content)?.content
          : ''
        const currentHtml = htmlFile || sourcePage.html || ''
        const pageData = extractPageData(sourcePage.files || [])
        const repairedHtml = legacyImageToCodeScreenshotShell(sourcePage, currentHtml)
          ? renderImageToCodeStaticHtml(sourcePage)
          : pageData && isEmptyGeneratedShell(currentHtml)
            ? renderStaticHtmlFromPageData(pageData, sourcePage)
            : currentHtml
        const repairedFiles = repairedHtml !== currentHtml
          ? ensureIndexHtmlFile(sourcePage.files || [], repairedHtml)
          : sourcePage.files
        return {
          ...sourcePage,
          html: repairedHtml,
...
function legacyImageToCodeScreenshotShell(page = {}, html = '') {
  ...
  const isAnalysisReportShell = /class=["'][^"']*\bscreenshot-restore-page\b/i.test(source)
    || /视觉结构解析|Mermaid 图表|RICE 表格|JSON 预览|截图还原 HTML/.test(source)
  const isFakePreviewShell = /class=["'][^"']*\bimage-to-html-page\b/i.test(source)
    && /页面验收面板|可检查画布|整图嵌入/.test(source)
  if (isAnalysisReportShell) return true
  if (isFakePreviewShell) return true
...
function renderImageToCodeStaticHtml(page = {}) {
  ...
  const prompt = page.captureResult?.textBlocks?.find((item) => item?.text)?.text || '当前资产没有真实视觉模型生成的 HTML，不能作为预览效果。'
  return `<!doctype html>
<html lang="zh-CN">
<head>
  <meta charset="UTF-8" />
  ...
  <style>
    * { box-sizing: border-box; }
    body { margin: 0; min-height: 100vh; display: grid; place-items: center; background: #f6f7f9; color: #222529; font-family: "PingFang SC", "Microsoft YaHei", Arial, sans-serif; padding: 32px; }
    .image-html-missing-preview { width: min(720px, 100%); border: 1px solid #e4e7eb; border-radius: 18px; background: #fff; box-shadow: 0 24px 70px rgba(34, 37, 41, .08); padding: 32px; }
    .badge { display: inline-flex; border-radius: 999px; background: #fff7ed; color: #c2410c; padding: 7px 11px; font-size: 12px; font-weight: 800; }
    h1 { margin: 18px 0 12px; font-size: 30px; line-height: 1.2; }
    p { margin: 0; color: #606873; font-size: 15px; line-height: 1.8; }
    .tips { margin-top: 20px; padding: 16px; border-radius: 12px; background: #f8fafc; color: #475569; }
  </style>
</head>
<body>
  <main class="image-html-missing-preview">
    <span class="badge">未生成真实预览</span>
    <h1>${escapeHtml(title)}</h1>
    <p>${escapeHtml(prompt)}</p>
    <div class="tips">这不是截图还原效果。当前资产只有旧报告壳或通用兜底壳，已阻止它继续冒充预览。请重新生成，并确保视觉模型返回真实单文件 HTML。</div>
  </main>
</body>
</html>`
}
...
function htmlPreviewFitStyle() {
  return '<style id="restored-preview-fit-style">html,body{min-height:100vh !important;height:auto !important;}body{margin:0 !important;overflow-x:hidden !important;}body>*:first-child,.app,.page,.container,.wrapper,main{min-height:100vh !important;height:auto !important;max-height:none !important;}[style*="height:"]{max-height:none !important;}</style>'
}

function withHtmlPreviewFitStyle(html = '') {
  const source = String(html || '')
  if (!source || source.includes('restored-preview-fit-style')) return source
  const style = htmlPreviewFitStyle()
...
function iframeSrcdoc(html = '') {
  return escapeHtml(withHtmlPreviewFitStyle(html))
}
...
export function createRestoredPageAsset({ projectId = '', clientTaskId = '', captureResult = {}, html = '', files = [], visualVerification = null } = {}) {
  ...
  return {
    id: clientRandomId(),
    projectId,
    clientTaskId: clientTaskId || captureResult.taskId || '',
    title,

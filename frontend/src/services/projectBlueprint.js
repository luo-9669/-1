...
function hasExplicitNonPodcastorDocumentContext(input = '', documents = []) {
  const text = uploadedContextText(input, documents)
  return hasJoggBrollContext(text)
}

function isPodcastorContext(project = {}, input = '', documents = []) {
  const uploadedText = uploadedContextText(input, documents)
  if (hasExplicitNonPodcastorDocumentContext(input, documents)) return false
...
function inferProductName(project = {}, input = '', documents = []) {
  ...
  const documentName = cleanProductName(extractDocumentTitle(input, documents))
  if (hasJoggBrollContext(contextText) && documentName) {
    return documentName
  }
  ...
  const productName = cleanProductName(explicit || documentName)
  if (
    productName &&
  ...
  return productName || project.name || '未命名项目'
}
...
function usesUploadedProductContext(project = {}, input = '', documents = []) {
  ...
  const explicitlyDifferentPodcastProduct =
    ...
    (/pets?\s*\/\s*cartoon\s+podcast|宠物和卡通播客|宠物\/卡通播客/i.test(context) ||
      hasExplicitNonPodcastorDocumentContext(input, documents))
  return Boolean(
    context &&
    productName &&
    project.name &&
...
export function exportBlueprintMarkdown(blueprint = {}) {
  ...
  const outlineDiff = blueprint?.outlineDiff || {}
  ...
  const actionDetail = (action = {}) => {
    const rect = action.rect || action.hotspot || {}
    const rectParts = ['x', 'y', 'width', 'height'].map((key) => formatNumber(rect[key]))
    const coordinate = rectParts.every((value) => value !== null)
      ? `坐标：x=${rectParts[0]}%, y=${rectParts[1]}%, w=${rectParts[2]}%, h=${rectParts[3]}%`
      : ''
    return [
      action.type || action.event ? `类型：${action.type || action.event}` : '',
      action.guard ? `守卫：${action.guard}` : '',
      action.field ? `字段：${action.field}` : '',
      action.value ? `值：${action.value}` : '',
      coordinate
    ].filter(Boolean).join('；')
  }
  const actionLine = (action = {}) => {
    const detail = action.detail || actionDetail(action)
    return `- 点击 ${action.label || action.name || '操作'} -> ${actionTarget(action)}${detail ? `；${detail}` : ''}`
  }
  const markdownCell = (value = '-') =>
    String(value || '-')
      ...
      .replace(/\n+/g, ' ')
      .trim() || '-'
  const coordinateText = (action = {}) => {
    const rect = action.rect || action.hotspot || {}
    const rectParts = ['x', 'y', 'width', 'height'].map((key) => formatNumber(rect[key]))
    return rectParts.every((value) => value !== null)
      ? `x=${rectParts[0]}%, y=${rectParts[1]}%, w=${rectParts[2]}%, h=${rectParts[3]}%`
      : '-'
  }
  const screenTitle = (screen = {}, index = 0) =>
    screen.title || screen.name || screen.pageName || screen.id || `页面 ${index + 1}`
  const screenActions = (screen = {}) => list(screen.actions || screen.hotspots)
  const interactionDetailTable = (screen = {}) => {
    const actions = screenActions(screen)
    const screenDescription = screen.description || screen.summary || screen.goal || '-'
    const rows = actions.length
      ? actions.map((action) => {
          const type = action.type || action.event || action.actionType || '-'
          const guard = action.guard || action.condition || action.permission || '-'
          const note = action.feedback || action.message || action.detail || screenDescription
          return `| ${markdownCell(action.label || action.name || '操作')} | ${markdownCell(type)} | ${markdownCell(actionTarget(action))} | ${markdownCell(guard)} | ${markdownCell(coordinateText(action))} | ${markdownCell(note)} |`
        })
      : [`| 暂无点击热区 | - | - | - | - | ${markdownCell(screenDescription)} |`]
    return [
      '| 点击对象 | 类型 | 目标 | 守卫/条件 | 点击区域 | 说明 |',
      '| --- | --- | --- | --- | --- | --- |',
      ...rows
    ].join('\n')
  }
  const renderPageInteractionDetails = (screens = []) => screens.length
    ? screens.map((screen, index) => [
        `### ${screenTitle(screen, index)}`,
        screen.route || screen.url || screen.path ? `页面路径：\`${screen.route || screen.url || screen.path}\`` : '页面路径：`待补充`',
        '',
        interactionDetailTable(screen)
      ].join('\n')).join('\n\n')
    : '暂无页面交互细节。'
  const renderInteractionPathSummary = (screens = []) => screens.length
    ? screens.flatMap((screen, index) => {
        const title = screenTitle(screen, index)
        const actions = screenActions(screen)
        if (!actions.length) return [`- ${title}：暂无可跳转交互`]
        return actions.map((action) => {
          const type = action.type || action.event || action.actionType || 'click'
          const guard = action.guard || action.condition || action.permission || '-'
          return `- ${title} -- ${action.label || action.name || '操作'} / ${type} / ${guard} --> ${actionTarget(action)}`
        })
      }).join('\n')
    : '- 暂无'
  const treeMarkdown = (tree, fallbackTitle) => tree
    ? renderTree(tree).join('\n')
    : `- ${fallbackTitle}`
  const demoScreens = list(blueprint?.demoScreens || blueprint?.pages)
  if (blueprint?.designMarkdown) {
    const sourceMarkdown = String(blueprint.designMarkdown || '').trim()
    if (/^##\s+页面交互细节$/m.test(sourceMarkdown) || !demoScreens.length) {
      return sourceMarkdown
    }
    return `${sourceMarkdown}

## 页面交互细节
${renderPageInteractionDetails(demoScreens)}

## 交互路径汇总
${renderInteractionPathSummary(demoScreens)}
`
  }
  return `# ${blueprint.title || profile.productName || '项目蓝图'}

## 项目档案
- 产品名称：${profile.productName || blueprint?.title || '未命名项目'}
- 产品定位：${profile.positioning || blueprint?.summary || '暂无'}
- 目标人群：${profile.targetUsers || '暂无'}
- 核心目标：${profile.primaryGoal || blueprint?.goal || '暂无'}
- 当前阶段：${profile.stage || '暂无'}

## 产品框架
${treeMarkdown(blueprint.framework, blueprint?.title || profile.productName || '产品结构待补充')}

## 原大纲 vs 新需求
### 原大纲
${list(outlineDiff.original).map((item) => `- ${item}`).join('\n') || '- 暂无'}

### 新增
${list(outlineDiff.added).map((item) => `- ${item}`).join('\n') || '- 暂无'}

### 修改
${list(outlineDiff.changed).map((item) => `- ${item}`).join('\n') || '- 暂无'}

### 待确认
${list(outlineDiff.pending).map((item) => `- ${item}`).join('\n') || '- 暂无'}

## 交互路径树
${treeMarkdown(blueprint.interactionTree, '交互路径待补充')}
...
## 可交互 Demo
${demoScreens.length ? demoScreens.map((screen) => `### ${screen.title || screen.name || screen.id || '未命名页面'}\n${renderWireframe(screen) || screen.description || '暂无页面描述'}\n\n点击细节：\n${list(screen.actions || screen.hotspots).map(actionLine).join('\n') || '- 暂无'}`).join('\n\n') : '暂无可交互 Demo。'}

## 评审清单
${list(blueprint.reviewChecklist).map((item) => `- ${item}`).join('\n') || '- 暂无'}
`
}

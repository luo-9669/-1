...
function normalizeProjectList(projects, ownerUserId = DEFAULT_USER_ID, options = {}) {
  const includeDefaultProject = options.includeDefaultProject !== false
  const list = asArray(projects).map((project) => createProject({
    ...project,
    ownerUserId: project.ownerUserId || ownerUserId
  }))
  if (!includeDefaultProject) return list
...
export function normalizeWorkspaceState(state = {}, options = {}) {
  ...
  const includeDefaultProject = options.includeDefaultProject !== false
  const projects = normalizeProjectList(state.projects, currentUserId, { includeDefaultProject })
  ...
  PROJECT_SCOPED_KEYS.forEach((key) => {
    const fallbackScopedProjectId = currentProjectId || (includeDefaultProject ? DEFAULT_PROJECT_ID : '')
    normalized[key] = asArray(state[key]).map((item) => withProjectId(item, fallbackScopedProjectId))
  })

  if (normalized.activeWorkflowRun && normalized.activeWorkflowRun.demandScope !== 'non-project' && !normalized.activeWorkflowRun.projectId) {
...
function workflowRunBelongsToProject(run = {}, projectId = '') {
  if (!run?.id) return false
  const runProjectId = run?.projectId || ''
  return run?.demandScope !== 'non-project' && Boolean(runProjectId) && runProjectId === projectId
}

function latestProjectRun(runs = [], projectId = '') {
  return asArray(runs)
    .filter((run) => workflowRunBelongsToProject(run, projectId))
    .slice()
...
export function reconcileProjectSelection(state = {}, patch = {}) {
  ...
  const activeWorkflowRun = workflowRunBelongsToProject(next.activeWorkflowRun, currentProjectId)

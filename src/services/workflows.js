...
const podcastProductFlow = {
  ...
  assetType: 'Podcastor 产品方案',
  description: '面向 AI 播客工作站，从创作者需求、首页入口、核心编辑器、资产复用、分发增长到验收指标逐步拆解。',
  recommendedFor: ['podcast', '播客', 'Podcastor', '内容创作', 'AI 播客', '创作者工作流'],
  steps: [
    {
...
## 前端文件
- src/App.vue：流程运行页、AI 分步工作台、当前步骤任务卡、AI 产出确认动作。
- src/styles.css：步骤状态、锁定原因、工作台布局、交付文件清单样式。
- src/services/workflowWorkbench.js：前端工作台视图模型，负责状态文案、锁定原因、默认输入和主动作。
- src/services/projectBlueprint.js：页面蓝图、交互路径、Demo schema 和前端交付节点。
- src/services/api.js：调用后端文档解析、工作流生成、资产保存、竞品分析等接口。

## 后端文件
- 后端/README.md：后端开发流程、目录边界、接口规范和测试约定。
- 后端/services/document-parser.js：文档、URL、音频解析任务，输出结构化需求材料。
- 后端/services/workflow-generator.js：根据项目资料生成步骤草稿、候选方案和版本记录。
- 后端/services/asset-store.js：保存蓝图、Demo、运行记录和项目资产。
- 后端/routes/workflows.js：工作流运行、采纳、重新生成、完成流程 API。
- 后端/routes/uploads.js：上传文件、多文件解析、失败原因和恢复建议 API。
- 后端/routes/competitors.js：竞品流程采集、公开资料分析、参考点和差异化建议 API。`
}

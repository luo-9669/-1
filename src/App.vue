        ...
        <div v-if="workflowAgentConfirmPreview.open" class="modal-backdrop" @click.self="closeWorkflowAgentConfirmPreview">
          <section class="agent-confirm-preview" role="dialog" aria-modal="true" aria-label="确认写入画布">
            <div class="modal-head">
              ...
              <button type="button" aria-label="关闭确认预览" @click="closeWorkflowAgentConfirmPreview">关闭</button>
            </div>
...
const isFactoryView = computed(() =>
  ...
  (typeof window !== 'undefined' && isFactoryHash(window.location.hash))
)
const isSidebarHidden = computed(() =>
  isFactoryView.value &&
  (state.currentFactoryRoute === 'capture-detail' || (typeof window !== 'undefined' && (
    window.location.hash === '#/factory/capture' ||
    parseProjectScopedHash(window.location.hash)?.route === '/factory/capture'
  )))
)
...
async function openRestoredPageStandalone(pageId) {
  ...
  const resultHtml = buildStaticHtmlResultShell(html, page.title, {
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    .generation-shell { width: min(760px, calc(100vw - 56px)); display: grid; grid-template-columns: 1fr; }
    .code-rain { min-height: 360px; border: 1px solid ${tone === 'failed' ? '#f0b6b6' : '#20252b'}; border-radius: 16px; padding: 0; overflow: hidden; background: #101318; color: #d8f7df; box-shadow: 0 28px 84px rgba(34,37,41,.18); }
    ...
    .steps { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 8px; padding: 14px 18px 0; }
...
function buildStaticHtmlFailurePage(title = '生成失败', message = '图片转代码失败，请稍后重试。') {
  return buildStaticHtmlStatusPage(title, message, { heading: title, tone: 'failed' })
}

function buildStaticHtmlResultShell(html, title = '静态 HTML 还原结果', options = {}) {
  const safeTitle = escapeHtml(title)
  const sourceCode = escapeHtml(html)
  const previewSrcdoc = escapeHtmlAttr(html)
...
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>${safeTitle} · HTML 结果</title>
  <style>
    * { box-sizing: border-box; }
    body { margin: 0; min-height: 100vh; background: #f6f7f9; color: #222529; font-family: "PingFang SC", "Microsoft YaHei", Arial, sans-serif; }
    .result-shell-standalone { min-height: 100vh; display: grid; grid-template-rows: 80px minmax(0, 1fr); }
    ...
    .result-split-layout { width: 100%; max-width: 1760px; min-height: calc(100vh - 80px); display: grid; grid-template-columns: minmax(0, 1fr) minmax(0, 1fr); gap: 16px; padding: 18px 24px 24px; margin: 0 auto; }
    .preview-pane, .code-pane { min-width: 0; display: grid; grid-template-rows: 48px minmax(0, 1fr); border: 1px solid #e8eaec; border-radius: 12px; background: #fff; overflow: hidden; }
    ...
    .generated-preview-frame { width: 100%; height: 100%; min-height: 680px; border: 0; background: #fff; display: block; }
    #generated-source-code { margin: 0; height: 100%; min-height: 680px; padding: 18px 22px 42px; overflow: auto; background: #fff; color: #222529; font: 13px/1.75 ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; white-space: pre-wrap; overflow-wrap: anywhere; word-break: break-word; tab-size: 2; }
  ...
  <main class="result-shell-standalone" data-task-id="${escapeHtmlAttr(taskId)}">
    <header class="result-topbar">
      ...
      <div class="topbar-actions">
        <button class="primary" type="button" id="convert-vue">转 Vue 代码</button>
        <a id="download-source" download="${escapeHtmlAttr(downloadName)}" href="#">下载 HTML</a>
      </div>
    </header>
    <section class="result-split-layout">
      <article class="preview-pane">
        ...
        <iframe class="generated-preview-frame" title="HTML 生成效果" sandbox="allow-forms" srcdoc="${previewSrcdoc}"></iframe>
      </article>
  ...
  <script>
    ...
    download.href = blobUrl;
	    backHome.addEventListener('click', () => {
	      ...
	      else window.location.href = '/#${escapeHtmlAttr(homeUrl)}';
	    });
    convertVue.addEventListener('click', () => {
      toast.textContent = '请回到流程通首页，在还原资产中执行转 Vue 代码';
...
function restoreWorkflowAnalysisFromUrl(projectRoute = parseProjectScopedHash(window.location.hash)) {
  ...
  const match = projectRoute?.route
    ...
    : window.location.hash.match(/^#\/workflow\/analysis\/([^/?#]+)/)
...
async function hydrateWorkspaceFromBackend() {
  ...
  const next = reconcileProjectSelection(normalizeWorkspaceState({
    ...
    currentProjectId: result.data.currentProjectId || state.currentProjectId,
...
async function generatePageFromCapture(options = {}) {
  ...
  if (previewWindow) {
    previewWindow.document.write(`<!doctype html><html lang="zh-CN"><head><meta charset="UTF-8"><title>页面生成中</title><style>body{margin:0;font-family:"PingFang SC","Microsoft YaHei",Arial,sans-serif;background:#fff;color:#222529;display:grid;place-items:center;min-height:100vh}.box{width:min(520px,calc(100vw - 40px));background:#fff;border:1px solid #e8eaec;border-radius:12px;padding:28px;box-shadow:0 24px 72px rgba(34,37,41,.08)}strong{display:block;font-size:24px;margin-bottom:10px}p{color:#7f8792;line-height:1.7}</style></head><body><main class="box"><strong>页面生成中</strong><p>流程通正在根据网页快照生成完整页面，生成完成后会自动展示在这里。</p></main></body></html>`)
    previewWindow.document.close()
  }
  try {
      ...
      onEvent: (event) => {
        ...
        if (event.type === 'artifact' && event.data?.html) {
          ...
          if (previewWindow && !previewWindow.closed) {
            previewWindow.document.open()
            previewWindow.document.write(event.data.html)
            previewWindow.document.close()
          }
        }
      }
...
function ensureWorkflowAgentSession() {
  const scopeId = workflowAgentScopeId()
...
function ensureWorkflowReferenceFiles() {
  const scopeId = workflowAgentScopeId()
  if (!state.activeWorkflowRun || !scopeId) return []
...
function workflowAgentReferencePayload() {
  return ensureWorkflowReferenceFiles().filter(isWorkflowAgentReferenceFile)
}
...
function replaceWorkflowAgentMessage(messageId, nextMessage) {
  const session = ensureWorkflowAgentSession()
...
function workflowAgentMessageById(messageId) {
  return ensureWorkflowAgentSession().find((message) => message.id === messageId) || null
}

function isWorkflowAgentStructuredStreamContent(content = '') {
  const value = String(content || '').trim()
  if (!value) return false
  if (value === '正在整理结构化建议...') return true
...
function workflowAgentStreamDisplayContent(content = '') {
  ...
  return isWorkflowAgentStructuredStreamContent(value) ? '正在整理结构化建议...' : value
}
...
function mergeWorkflowAgentStreamedAssistantMessage(assistantMessage = {}, streamedContent = '') {
  ...
  if (isWorkflowAgentStructuredStreamContent(visibleContent)) return normalized
...
async function persistWorkflowAgentMessage(message, options = {}) {
  ...
  const result = await api.workflows.appendMessage(
    ...
    scopeId,
    {
      ...
      retrievedKnowledge,
      references: workflowAgentReferencePayload(),
...
async function sendWorkflowAgentMessage(text = workflowAgentInput.value, options = {}) {
  ...
  if (!content || !state.activeWorkflowRun || workflowAgentSending.value) return
  ...
  try {
    ...
    const knowledgeResult = await retrieveWorkflowKnowledge(content, workflowAgentScopeId())
    ...
    if (backendHandled) {
      const currentPendingMessage = workflowAgentMessageById(pendingMessageId)
      const assistantMessage = normalizeWorkflowAgentAssistantMessage(mergeWorkflowAgentStreamedAssistantMessage(persisted.data.assistantMessage, workflowAgentMessageText(currentPendingMessage)), {
        clientMessageId,
        retryOfMessageId,
        editOfMessageId,
        action: messageAction,
      ...
      await typeWorkflowAgentAssistantMessage(pendingMessageId, assistantMessage, { requestToken })
      workflowAgentPendingMessageId.value = ''
      clearWorkflowAgentActiveDraft()
      return
    } else {
      const backendFailureMessage = workflowAgentFailedMessage(persisted) || '后端响应缺少 assistantMessage'
      replaceWorkflowAgentMessage(pendingMessageId, {
        role: 'assistant',
        ...
        meta: {
          ...
          error: {
            ...
            code: persisted?.status || persisted?.data?.error?.code || 'AGENT_RESPONSE_MISSING',
            recoveryActions: persisted?.data?.error?.recoveryActions || ['重试', '检查模型配置']
          }
        }
      })
      workflowAgentPendingMessageId.value = ''
...
async function confirmWorkflowAgentMessage(message) {
  ...
  try {
    const result = await api.workflows.confirmProposalStream(state.apiConfig, state.activeWorkflowRun.id, proposalId, {
      nodeId: targetNodeId,
      conversationId: workflowAgentScopeId(),
...
async function stopWorkflowAgentGeneration() {
  ...
  const pendingMessage = workflowAgentPendingMessageId.value
    ? ensureWorkflowAgentSession().find((message) => message.id === workflowAgentPendingMessageId.value)
    : null

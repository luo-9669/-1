...
const JSZip = frontendRequire('jszip')
import {
  acceptCurrentStep,
...
import { buildSkillExecutionContext, createSystemSkills, normalizeSkill } from '../frontend/src/services/skills.js'
import {
...
} from '../frontend/src/services/projectBlueprint.js'
import { buildDocumentAnalysisView } from '../frontend/src/services/documentAnalysis.js'
...
import { runSkillGeneration, safeParseModelJson } from '../backend/services/generation-runner.js'
...
import { importLocalDocuments, normalizeExtractedText } from '../frontend/src/services/documentImport.js'
...
import { compactWorkspaceStateForStorage, saveState } from '../frontend/src/services/storage.js'
import {
  captureQualityGate,
...
} from '../frontend/src/services/factoryWorkspace.js'
import {
  buildSnapshotCaptureRequest,
  normalizeSnapshotAuthMode,
  snapshotCaptureStatusMessage
} from '../frontend/src/services/snapshotCapture.js'
import {
  designSourceFromCaptureResult,
  designSourcesForProject,
  upsertDesignSource
} from '../frontend/src/services/designSources.js'
import { parseWebsnapFile } from '../frontend/src/services/websnap.js'
import {
  buildVisualVerificationReport,
  visualVerificationPassed
} from '../frontend/src/services/visualVerification.js'
...
} from '../frontend/src/services/knowledgeHub.js'
import {
...
} from '../frontend/src/services/knowledgeDeposit.js'
import {
  buildBlueprintWorkbench,
  visibleBlueprintFrameNodes
} from '../frontend/src/services/blueprintWorkbench.js'
import { getSidebarNavItems } from '../frontend/src/services/navigation.js'
import {
  createWorkspaceStore,
  workspaceRoutes
} from '../backend/routes/workspace.js'
import {
  saveModelSettings,
  addCompetitor,
  listCompetitors,
...
import { captureRoutes } from '../backend/routes/capture.js'
import { createWorkflowRunnerStore, workflowRoutes } from '../backend/routes/workflows.js'
import { buildAgentContext } from '../backend/services/agent-context-builder.js'
import {
  createAgentProviderFromModelSettings,
  createDeterministicAgentProvider,
  createOpenAICompatibleAgentProvider,
  normalizeModelProviderError
} from '../backend/services/llm-provider.js'
import { generateAgentReply } from '../backend/services/agent-service.js'
import { buildAdminSummary } from '../backend/services/admin-dashboard.js'
import { adminRoutes } from '../backend/routes/admin.js'
import { renderAdminConsoleHtml } from '../backend/services/admin-console.js'
import { createBrowserSessionManager } from '../backend/server/browser-session-manager.mjs'
import { matchRoute, routes } from '../backend/server/mock-api.mjs'
import {
...
test('project separates frontend and backend code into dedicated folders', () => {
  const root = process.cwd()
  const directoryExists = (relativePath) => {
  ...
  assert.ok(directoryExists('frontend'), 'frontend folder should exist')
  assert.ok(directoryExists('backend'), 'backend folder should exist')
  assert.ok(fileExists('frontend/src/main.js'), 'frontend entry should live under frontend/src')
  assert.ok(fileExists('frontend/vite.config.js'), 'frontend Vite config should live under frontend')
  assert.ok(fileExists('backend/server/mock-api.mjs'), 'backend API entry should live under backend/server')
  assert.ok(fileExists('backend/routes/workspace.js'), 'backend routes should live under backend/routes')
  assert.ok(fileExists('scripts/start-dev.mjs'), 'workspace startup script should live under scripts')
  assert.equal(directoryExists('src'), false, 'root src folder should be moved into frontend')
  assert.equal(directoryExists('server'), false, 'root server folder should be moved into backend')
  assert.equal(directoryExists('后端'), false, 'legacy backend folder should be moved into backend')
  assert.match(packageJson.scripts.dev, /npm --prefix frontend run dev/)
  assert.match(packageJson.scripts.api, /npm --prefix backend run api/)
  assert.match(packageJson.scripts['dev:all'], /node scripts\/start-dev\.mjs/)
  assert.match(packageJson.scripts['start:all'], /node scripts\/start-dev\.mjs/)
  assert.match(packageJson.scripts.test, /tests\/workflows\.test\.mjs/)
})

testAsync('workspace store persists competitor monitor domain records', async () => {
  const workspace = createWorkspaceStore(null, { filePath: '' })
...
test('workspace startup script launches frontend and backend projects together', () => {
  const root = process.cwd()
  const source = readFileSync(join(root, 'scripts/start-dev.mjs'), 'utf8')

  assert.match(source, /BACKEND_PORT/)
  assert.match(source, /FRONTEND_PORT/)
  assert.match(source, /VITE_API_PROXY_TARGET/)
  assert.match(source, /execFile/)
  assert.match(source, /pidsForPort/)
  assert.match(source, /await stopPort\(BACKEND_PORT\)/)
  assert.match(source, /await stopPort\(FRONTEND_PORT\)/)
...
test('root folder keeps only shared project files', async () => {
  ...
  assert.deepEqual(unexpected.sort(), [])
  assert.equal(existsSync(join(root, 'backend/design.md')), true)
  assert.equal(existsSync(join(root, 'backend/storage')), true)
  assert.equal(existsSync(join(root, 'frontend/package.json')), true)
  assert.equal(existsSync(join(root, 'frontend/package-lock.json')), true)
  assert.equal(existsSync(join(root, 'backend/package.json')), true)
  assert.equal(existsSync(join(root, 'backend/package-lock.json')), true)
...
testAsync('sidebar exposes competitor monitoring below skill center', async () => {
  ...
  const navSource = await readFile(new URL('../frontend/src/services/navigation.js', import.meta.url), 'utf8')
...
testAsync('app shell maps every primary page to a stable hash route', async () => {
  ...
  assert.match(appSource, /const currentLocationHash = ref\(/)
  assert.match(appSource, /function refreshCurrentLocationHash\(\)/)
  ...
  assert.match(appSource, /const routeHash = currentLocationHash\.value/)
  ...
  assert.match(appSource, /if \(route\.startsWith\('\/workflow'\)\) return restoreWorkflowRouteFromUrl\(projectRoute\)/)
...
testAsync('project account switch persists workspace before full page reload', async () => {
  ...
  assert.match(appSource, /function currentProjectRouteForView\(\)/)
  assert.match(appSource, /function replaceCurrentProjectRouteProjectId\(projectId\)/)
  ...
  assert.match(selectProjectSource, /replaceCurrentProjectRouteProjectId\(projectId\)[\s\S]*reloadAfterProjectAccountSwitch\(\)/)
...
testAsync('app shell cache-busts entry module when shell styles change', async () => {
  ...
  assert.match(html, /\/src\/main\.js\?v=workflow-agent-stream-ui-v2/)
...
testAsync('vite build separates vendor chunks to avoid a single oversized app bundle', async () => {
  const configSource = await readFile(new URL('../frontend/vite.config.js', import.meta.url), 'utf8')

  assert.match(configSource, /manualChunks/)
  assert.match(configSource, /vue-vendor/)
  assert.match(configSource, /document-vendor/)
  assert.doesNotMatch(configSource, /automation-vendor/)
  assert.doesNotMatch(configSource, /playwright-core/)
  assert.doesNotMatch(configSource, /single-file-cli/)
})
...
test('mock api exposes admin console on HEAD requests for health checks', async () => {
  const admin = await routes['HEAD /admin']()
...
testAsync('requirements workspace uses source tabs table fields and knowledge state', async () => {
  ...
  assert.match(requirementsSource, /toggleMaterialBatchMode[\s\S]*openMaterialCreate/)
  assert.match(cssSource, /\.requirements-workspace/)
  assert.match(cssSource, /\.requirements-table/)
  assert.match(cssSource, /\.materials-toolbar-actions \.primary,\s*\.knowledge-hub-head \.actions \.primary\s*\{[\s\S]*?order:\s*10;/)
})
...
testAsync('image to html requires vision model html instead of creating a fake preview', async () => {
  ...
  assert.equal(result.html, '')
  assert.equal(result.restoredPage, null)
  assert.match(result.message, /视觉模型未返回真实 HTML/)
  assert.equal(result.visualVerification.status, 'failed')
})

testAsync('image to html fallback does not turn screenshots into fake preview pages', async () => {
  const result = await routes['POST /api/generate/image-to-html']({
    projectId: 'project-flow',
    title: 'real-png-check.png',
    target: 'static-html',
    prompt: '工作流分析详情页，必须保留代码功能流程、路径节点、Mermaid 图表、RICE 表格和 JSON 预览。',
    imageDataUrl: solidPngDataUrl(320, 180, [240, 248, 255, 255])
  })
  ...
  assert.equal(result.html, '')
  assert.equal(result.restoredPage, null)
  assert.doesNotMatch(result.html, /代码功能流程/)
  assert.doesNotMatch(result.html, /路径节点/)
  assert.doesNotMatch(result.html, /开始节点/)
  assert.doesNotMatch(result.html, /处理节点/)
  assert.doesNotMatch(result.html, /结束节点/)
  assert.doesNotMatch(result.html, /data-node-type="start"/)
  assert.doesNotMatch(result.html, /JSON 预览/)
  assert.doesNotMatch(result.html, /Mermaid 图表/)
  assert.doesNotMatch(result.html, /RICE 表格/)
  assert.doesNotMatch(result.html, /页面验收面板/)
})
...
testAsync('image to code calls backend vision html generation before saving restored asset', async () => {
  ...
  assert.match(appSource, /function imageToHtmlRequestTimeoutMs\(\)/)
  assert.match(appSource, /Math\.max\(300000,\s*modelTimeoutMs\s*\+\s*120000\)/)
  assert.match(generateSource, /timeoutMs:\s*imageToHtmlRequestTimeoutMs\(\)/)
  assert.doesNotMatch(generateSource, /timeoutMs:\s*workflowAnalysisRequestTimeoutMs\(\)/)
...
testAsync('image to code opens standalone browser result page without app sidebar', async () => {
  ...
  assert.match(generateSource, /writeStaticHtmlPreviewWindow\(previewWindow,\s*buildStaticHtmlLoadingPage[\s\S]*?\)\)\s*\n\s*updateStaticHtmlPreviewUrl\(previewWindow,\s*clientTaskId\)/)
  assert.match(generateSource, /writeStaticHtmlPreviewWindow\(previewWindow,\s*buildStaticHtmlStatusPage[\s\S]*?\)\)\s*\n\s*updateStaticHtmlPreviewUrl\(previewWindow,\s*clientTaskId\)/)
  ...
  assert.match(generateSource, /interactive:\s*true/)
  ...
  assert.match(previewSource, /projectScopedUrl\(`\/assets\/\$\{safeTaskId\}\/preview`\)/)
  assert.doesNotMatch(previewSource, /about:blank#/)
...
testAsync('image to code standalone result splits generated preview and code', async () => {
  ...
  assert.match(previewSource, /function buildStaticHtmlResultShell\(html,\s*title/)
  assert.match(previewSource, /<title>流程通 · HTML 还原结果<\/title>/)
  assert.match(previewSource, /<link rel="icon" href="\/favicon\.svg"/)
  assert.match(previewSource, /返回首页/)
  assert.match(previewSource, /转 Vue 代码/)
  assert.match(previewSource, /下载 HTML/)
  assert.match(previewSource, /<a[^>]*class="primary"[^>]*id="download-source"[^>]*>下载 HTML<\/a>[\s\S]*<button(?![^>]*class="primary")[^>]*id="convert-vue"[^>]*>转 Vue 代码<\/button>/)
  assert.match(previewSource, /<iframe id="generated-preview-frame" class="generated-preview-frame" title="HTML 生成效果" sandbox="allow-forms allow-scripts"><\/iframe>/)
  assert.match(previewSource, /<pre id="generated-source-code"/)
  assert.doesNotMatch(previewSource, /const previewSrcdoc = escapeHtmlAttr\(html\)/)
  assert.doesNotMatch(previewSource, /srcdoc="\$\{previewSrcdoc\}"/)
  assert.match(previewSource, /const previewFrame = document\.getElementById\('generated-preview-frame'\)/)
  assert.doesNotMatch(previewSource, /const previewBlobUrl = URL\.createObjectURL/)
  assert.doesNotMatch(previewSource, /previewFrame\.src = previewBlobUrl/)
  assert.match(previewSource, /previewFrame\.srcdoc = html/)
  ...
  assert.match(previewSource, /height:\s*calc\(100vh - 80px\)/)
  assert.match(previewSource, /\.preview-pane,\s*\.code-pane\s*\{[\s\S]*?height:\s*100%/)
  assert.match(previewSource, /\.preview-pane,\s*\.code-pane\s*\{[\s\S]*?overflow:\s*hidden/)
  assert.match(previewSource, /\.generated-preview-frame\s*\{[\s\S]*?min-height:\s*0/)
  assert.match(previewSource, /\.generated-preview-frame\s*\{[\s\S]*?overflow:\s*auto/)
  assert.match(previewSource, /#generated-source-code\s*\{[\s\S]*?min-height:\s*0/)
  ...
  assert.match(previewSource, /const backTargetUrl = '\/#\$\{escapeHtmlAttr\(homeUrl\)\}'/)
  assert.match(previewSource, /const convertTargetUrl = '\$\{escapeHtmlAttr\(convertUrl\)\}'/)
  assert.match(previewSource, /backHome\.addEventListener\('click',\s*\(\)\s*=>\s*\{[\s\S]*window\.top\.location\.href = backTargetUrl[\s\S]*window\.location\.href = backTargetUrl/)
  assert.match(previewSource, /convertVue\.addEventListener\('click',\s*\(\)\s*=>\s*\{[\s\S]*window\.top\.location\.href = convertTargetUrl[\s\S]*window\.location\.href = convertTargetUrl/)
  assert.doesNotMatch(previewSource, /请回到流程通首页，在还原资产中执行转 Vue 代码/)
  assert.doesNotMatch(previewSource, /URL\.revokeObjectURL\(previewBlobUrl\)/)
  assert.doesNotMatch(previewSource, /window\.close\(\)/)
  ...
  assert.match(generateSource, /convertUrl:\s*projectScopedUrl\(`\/assets\/\$\{encodeURIComponent\(restoredPage\.id\)\}\/preview\?action=convert-vue`/)
  assert.doesNotMatch(generateSource, /closeStaticHtmlPreviewWindow\(previewWindow\)/)
...
testAsync('mock image to html generation fails instead of saving fake preview without vision model', async () => {
  ...
  assert.equal(result.html, '')
  assert.match(result.message, /视觉模型未返回真实 HTML|无法生成真实预览/)
...
testAsync('image to html generation uses vision model output when provider is available', async () => {
  const { createMockApiRoutes } = await import('../backend/server/mock-api.mjs')
  let receivedContext = null
  let verifiedHtml = ''
  let verifiedCaptureResult = null
  const provider = {
  ...
  const testRoutes = createMockApiRoutes({
    agentProvider: provider,
    verifyGeneratedPage: async (captureResult, html) => {
      verifiedCaptureResult = captureResult
      verifiedHtml = html
      return {
        url: captureResult.url,
        status: 'passed',
        thresholds: { mode: 'backend-rendered-html' },
        summary: { total: 1, passed: 1, failed: 0, averageScore: 96 },
        results: [
          {
            id: 'desktop',
            width: captureResult.viewport.width,
            status: 'passed',
            generatedScreenshot: solidPngDataUrl(320, 240, [255, 122, 77, 255]),
            comparison: { score: 96 }
          }
        ],
        recommendations: ['后端已完成 Chromium 渲染验收。']
      }
    }
  })
  ...
  assert.equal(verifiedHtml, result.html)
  assert.equal(verifiedCaptureResult.raw.provider, 'fake-vision-model')
  assert.equal(result.visualVerification.thresholds.mode, 'backend-rendered-html')
  assert.equal(result.visualVerification.summary.averageScore, 96)
  assert.equal(result.restoredPage.visualVerification.thresholds.mode, 'backend-rendered-html')
  assert.equal(result.restoredPage.html, result.html)
})

testAsync('image to html generation refuses to save preview when backend render verification fails', async () => {
  const { createMockApiRoutes } = await import('../backend/server/mock-api.mjs')
  const provider = {
    name: 'fake-vision-model',
    async generate() {
      return {
        content: JSON.stringify({
          visualModel: { source: { type: 'screenshot', title: '坏 HTML', target: 'static-html', hasImage: true } },
          html: '<!doctype html><html><body><main>bad page</main></body></html>',
          summary: '模型返回了 HTML。'
        }),
        provider: 'fake-vision-model',
        model: 'vision-test',
  ...
  const testRoutes = createMockApiRoutes({
    ...
    verifyGeneratedPage: async (captureResult) => ({
      ...
      results: [],
      recommendations: ['后端 Chromium 渲染失败：页面脚本报错。']
    })
  })
  ...
  const result = await testRoutes['POST /api/generate/image-to-html']({
    ...
    title: '坏 HTML',
    target: 'static-html',
    imageDataUrl: solidPngDataUrl(320, 240, [8, 8, 8, 255])
  })
  ...
  assert.equal(result.restoredPage, null)
  assert.equal(result.visualVerification.status, 'failed')
  assert.match(result.message, /后端渲染验收失败/)
})

testAsync('image to html retries chat completions when responses image input returns upstream 502', async () => {
  const { createMockApiRoutes } = await import('../backend/server/mock-api.mjs')
  let primaryCalled = false
  let fallbackCalled = false
  const imageDataUrl = solidPngDataUrl(320, 240, [123, 45, 67, 255])
  const primaryProvider = {
    name: 'responses-vision-model',
    async generate() {
      primaryCalled = true
  ...
  const fallbackProvider = {
    ...
    async generate(context) {
      ...
      return {
        content: JSON.stringify({
          visualModel: {
            source: { type: 'screenshot', title: '备用视觉', target: 'static-html', hasImage: true },
            pageType: '备用视觉模型页面'
          },
          html: '<!doctype html><html><body><main class="chat-vision-page">备用接口生成</main></body></html>',
          summary: '已用 chat.completions 备用视觉接口生成 HTML。'
        }),
        provider: 'chat-vision-model',
        model: 'vision-chat-test',
  ...
  const testRoutes = createMockApiRoutes({
    ...
    imageVisionFallbackProvider: fallbackProvider,
    verifyGeneratedPage: async (captureResult, html) => ({
      url: captureResult.url,
      status: 'passed',
      thresholds: { mode: 'backend-rendered-html' },
      summary: { total: 1, passed: 1, failed: 0, averageScore: 99 },
      results: [{ id: 'desktop', width: 1440, status: 'passed', comparison: { score: 99 } }],
      recommendations: []
    })
  })
  ...
  assert.equal(result.provider, 'chat-vision-model')
  assert.equal(result.captureResult.raw.provider, 'chat-vision-model')
  assert.equal(result.captureResult.raw.primaryVisionError.status, 502)
  assert.match(result.html, /chat-vision-page/)
  assert.equal(result.restoredPage.html, result.html)
})
...
testAsync('mock api defaults local runtime to Cicada LLM model settings', async () => {
  const apiSource = await readFile(new URL('../backend/server/mock-api.mjs', import.meta.url), 'utf8')

  assert.match(apiSource, /CICADA_LLM_MODEL_SETTINGS/)
  assert.doesNotMatch(apiSource, /PROXY_MANAGED/)
  assert.match(apiSource, /baseUrl:[\s\S]*'https:\/\/ai-platform-cicada-llm-api\.limayao\.com\/v1'/)
  ...
  assert.match(apiSource, /allowInsecureTLS:\s*true/)
  assert.match(apiSource, /Number\(settings\?\.timeoutMs\)\s*===\s*CICADA_LLM_MODEL_SETTINGS\.timeoutMs/)
  assert.match(apiSource, /await ensureCicadaModelSettings\(workspaceStore\)/)
  ...
  assert.match(apiSource, /return createAgentProviderFromModelSettings\(CICADA_LLM_MODEL_SETTINGS\)/)
  assert.match(apiSource, /createAgentProviderFromModelSettings\(settings,\s*options\.fetchImpl\)/)
  assert.doesNotMatch(apiSource, /options\.fetchImpl \|\| globalThis\.fetch/)
})

testAsync('mock api streams workflow agent sse events as they are generated', async () => {
  const apiSource = await readFile(new URL('../backend/server/mock-api.mjs', import.meta.url), 'utf8')
  const workflowRouteSource = await readFile(new URL('../backend/routes/workflows.js', import.meta.url), 'utf8')
...
testAsync('mock api converts thrown stream route failures into sse error and done events', async () => {
  const apiSource = await readFile(new URL('../backend/server/mock-api.mjs', import.meta.url), 'utf8')
  const uploadRoutesSource = await readFile(new URL('../backend/routes/uploads.js', import.meta.url), 'utf8')
  const captureRoutesSource = await readFile(new URL('../backend/routes/capture.js', import.meta.url), 'utf8')
...
testAsync('heavy generation APIs expose stream variants for incremental UI updates', async () => {
  const apiSource = await readFile(new URL('../frontend/src/services/api.js', import.meta.url), 'utf8')
  const mockSource = await readFile(new URL('../backend/server/mock-api.mjs', import.meta.url), 'utf8')
  const uploadRoutesSource = await readFile(new URL('../backend/routes/uploads.js', import.meta.url), 'utf8')
  const captureRoutesSource = await readFile(new URL('../backend/routes/capture.js', import.meta.url), 'utf8')
...
test('normalizing restored pages repairs legacy image-to-code screenshot shells', () => {
  ...
  assert.match(repaired.html, /image-html-missing-preview/)
  assert.match(repaired.html, /未生成真实预览/)
  assert.match(repaired.html, /请重新生成/)
  assert.doesNotMatch(repaired.html, /data:image\/png;base64,legacy/)
  assert.match(repaired.files.find((file) => file.path === 'index.html').content, /image-html-missing-preview/)
  assert.doesNotMatch(repaired.files.find((file) => file.path === 'index.html').content, /data:image\/png;base64,legacy/)
})

test('normalizing restored pages repairs legacy image-to-code analysis report shells', () => {
  const analysisShell = '<!doctype html><html><body><main class="screenshot-restore-page" data-restore-source="screenshot"><aside><span>视觉结构解析</span><span>Mermaid 图表</span><span>RICE 表格</span><span>JSON 预览</span></aside><section><h1>截图还原 HTML</h1></section></main></body></html>'
  const state = normalizeFactoryWorkspace({
    restoredPages: [{
      id: 'legacy-analysis-page',
      projectId: 'project-flow',
      title: 'wechat.png',
      sourceUrl: 'image://wechat.png',
      html: analysisShell,
      files: [{ path: 'index.html', content: analysisShell }],
      captureResult: {
        raw: { captureKind: 'image-to-code' },
        textBlocks: [{ text: '生成一个前端检查页面效果的静态页面' }]
      }
    }]
  })
  ...
  assert.match(repaired.html, /image-html-missing-preview/)
  assert.match(repaired.html, /未生成真实预览/)
  assert.match(repaired.html, /生成一个前端检查页面效果的静态页面/)
  assert.doesNotMatch(repaired.html, /screenshot-restore-page/)
  assert.doesNotMatch(repaired.html, /视觉结构解析/)
  assert.doesNotMatch(repaired.html, /Mermaid 图表/)
  assert.doesNotMatch(repaired.html, /RICE 表格/)
  assert.doesNotMatch(repaired.html, /JSON 预览/)
  assert.match(repaired.files.find((file) => file.path === 'index.html').content, /image-html-missing-preview/)
})

test('normalizing restored pages repairs screenshot analysis shells even without image metadata', () => {
  const analysisShell = '<!doctype html><html><body><main class="screenshot-restore-page"><aside><span>视觉结构解析</span><span>JSON 预览</span></aside><section><h1>截图还原 HTML</h1></section></main></body></html>'
  const state = normalizeFactoryWorkspace({
    restoredPages: [{
      id: 'legacy-analysis-no-meta',
      projectId: 'project-flow',
      title: 'wechat_2026-06-20_172903_098.png',
      html: analysisShell,
      files: [{ path: 'index.html', content: analysisShell }]
    }]
  })
...
test('normalizing restored pages blocks legacy fake image preview shells', () => {
  const fakePreviewShell = '<!doctype html><html><body><main class="image-to-html-page"><section class="panel"><div class="panel-title">页面验收面板</div><input value="结构化 DOM + 响应式 CSS + 可检查画布" /></section></main></body></html>'
  const state = normalizeFactoryWorkspace({
    restoredPages: [{
      id: 'legacy-fake-preview',
      projectId: 'project-flow',
      title: 'wechat_2026-06-20_172903_098.png',
      html: fakePreviewShell,
      files: [{ path: 'index.html', content: fakePreviewShell }]
    }]
  })
  ...
  assert.match(repaired.html, /image-html-missing-preview/)
  assert.match(repaired.html, /未生成真实预览/)
  assert.doesNotMatch(repaired.html, /页面验收面板/)
  assert.doesNotMatch(repaired.html, /可检查画布/)
})
...
testAsync('workflow canvas sidebar focuses nodes by centering the viewport', async () => {
  const source = await readFile(new URL('../frontend/src/components/workflow/WorkflowCanvasPage.vue', import.meta.url), 'utf8')
  const styles = await readFile(new URL('../frontend/src/styles.css', import.meta.url), 'utf8')

  assert.match(source, /ref="viewportRef"/)
  assert.match(source, /data-node-id/)
  assert.match(source, /const CANVAS_FOCUS_SAFE_GUTTER = 96/)
  assert.match(source, /const centeredLeft = Math\.max\(0,[\s\S]*nodeLeft[\s\S]*viewport\.clientWidth/)
  assert.match(source, /const actionSafeLeft = Math\.max\(0,\s*nodeLeft - CANVAS_FOCUS_SAFE_GUTTER\)/)
  assert.match(source, /const targetLeft = Math\.min\(centeredLeft,\s*actionSafeLeft\)/)
...
testAsync('workflow canvas keeps every node action above canvas layers before agent opens', async () => {
  const styles = await readFile(new URL('../frontend/src/styles.css', import.meta.url), 'utf8')

  assert.match(styles, /\.workflow-canvas-edges\s*\{[\s\S]*?pointer-events:\s*none/)
  assert.match(styles, /\.workflow-canvas-shell\s*\{[\s\S]*?position:\s*relative/)
  assert.match(styles, /\.workflow-canvas-tabs\s*\{[\s\S]*?position:\s*relative/)
  assert.match(styles, /\.workflow-canvas-tabs\s*\{[\s\S]*?z-index:\s*5/)
  assert.match(styles, /\.workflow-canvas-viewport\s*\{[\s\S]*?position:\s*relative/)
  assert.match(styles, /\.workflow-canvas-viewport\s*\{[\s\S]*?z-index:\s*1/)
  assert.match(styles, /\.workflow-canvas-surface\s*\{[\s\S]*?z-index:\s*1/)
  assert.match(styles, /\.canvas-node-card\s*\{[\s\S]*?z-index:\s*2/)
  assert.match(styles, /\.canvas-node-card\s*\{[\s\S]*?pointer-events:\s*auto/)
  assert.match(styles, /\.canvas-node-card\s*\{[\s\S]*?cursor:\s*pointer/)
  assert.match(styles, /\.canvas-node-card\.active\s*\{[\s\S]*?z-index:\s*3/)
  assert.match(styles, /\.canvas-node-actions\s*\{[\s\S]*?position:\s*relative/)
  assert.match(styles, /\.canvas-node-actions\s*\{[\s\S]*?z-index:\s*4/)
  assert.match(styles, /\.canvas-node-actions\s*\{[\s\S]*?pointer-events:\s*auto/)
  assert.match(styles, /\.canvas-node-actions button,[\s\S]*?\.canvas-node-footer button\s*\{[\s\S]*?position:\s*relative/)
  assert.match(styles, /\.canvas-node-actions button,[\s\S]*?\.canvas-node-footer button\s*\{[\s\S]*?pointer-events:\s*auto/)
})
...
testAsync('workflow canvas detail supports progressive tree expansion while small cards stay static', async () => {
  ...
  assert.match(styles, /\.canvas-node-card\s*\{[\s\S]*?max-height:\s*260px/)
  assert.match(styles, /\.canvas-node-card\s*\{[\s\S]*?overflow:\s*hidden/)
  assert.match(styles, /\.canvas-node-card p\s*\{[\s\S]*?-webkit-line-clamp:\s*3/)
  assert.match(styles, /\.canvas-node-content\s*\{[\s\S]*?min-height:\s*0/)
  assert.match(styles, /\.canvas-node-content\s*\{[\s\S]*?overflow-y:\s*auto/)
  assert.match(styles, /\.canvas-tree-node\s*\{[^}]*?height:\s*auto/)
  assert.match(styles, /\.canvas-tree-node\s*\{[\s\S]*?max-height:\s*128px/)
  assert.match(styles, /\.canvas-tree-node\s*\{[\s\S]*?align-items:\s*start/)
  assert.match(styles, /\.canvas-fullscreen-content\s+\.canvas-detail-tree\s*\{[\s\S]*?max-height:\s*clamp\(420px,\s*54vh,\s*620px\)/)
  assert.match(styles, /\.canvas-fullscreen-content\s+\.canvas-detail-tree\s*\{[\s\S]*?overflow-y:\s*auto/)
  assert.match(styles, /\.canvas-fullscreen-content\s+\.canvas-tree-node\s*\{[\s\S]*?max-height:\s*none/)
  assert.match(styles, /\.canvas-fullscreen-content\s+\.canvas-tree-node span\s*\{[\s\S]*?max-height:\s*none/)
  assert.match(styles, /\.canvas-fullscreen-content\s+\.canvas-tree-node span\s*\{[\s\S]*?overflow:\s*visible/)
...
testAsync('workflow agent drawer uses compact header upload toolbar and resizable launcher', async () => {
  const drawerSource = await readFile(new URL('../frontend/src/components/workflow/WorkflowAgentDrawer.vue', import.meta.url), 'utf8')
  const appSource = await readFile(new URL('../frontend/src/App.vue', import.meta.url), 'utf8')
  const workflowWorkbenchSource = await readFile(new URL('../frontend/src/services/workflowWorkbench.js', import.meta.url), 'utf8')
  const styles = await readFile(new URL('../frontend/src/styles.css', import.meta.url), 'utf8')
  ...
  assert.match(await readFile(new URL('../frontend/src/components/workflow/WorkflowCanvasPage.vue', import.meta.url), 'utf8'), /canvas-agent-launcher/)
...
testAsync('workflow canvas node header actions stop propagation so every node agent button opens directly', async () => {
  const source = await readFile(new URL('../frontend/src/components/workflow/WorkflowCanvasPage.vue', import.meta.url), 'utf8')
  const nodeCardClass = source.indexOf('class="canvas-node-card"')
  const nodeCardStart = source.lastIndexOf('<article', nodeCardClass)
  const nodeCardEnd = source.indexOf('</article>', nodeCardStart)
  const nodeCardSource = source.slice(nodeCardStart, nodeCardEnd)

  assert.match(nodeCardSource, /@click="\$emit\('focus-node',\s*node\.id\)"/)
  assert.match(nodeCardSource, /@click\.stop="\$emit\('open-agent',\s*node\.id\)"/)
  assert.match(nodeCardSource, /@click\.stop="\$emit\('fullscreen',\s*node\.id\)"/)
  assert.match(nodeCardSource, /@click\.stop="\$emit\('quick-action'/)
  assert.doesNotMatch(nodeCardSource, /<button type="button" :disabled="node\.loading" @click\.stop="\$emit\('open-agent'/)
  assert.doesNotMatch(nodeCardSource, /<button type="button" :disabled="node\.loading" @click\.stop="\$emit\('fullscreen'/)
  assert.match(nodeCardSource, /:aria-label="`聚焦画布节点：\$\{node\.title \|\| node\.id\}`"/)
})
...
testAsync('workflow agent messages use workspace scoped API route', async () => {
  ...
  assert.match(appSource, /const targetScopeId = options\.nodeId \|\| workflowAgentScopeId\(\)/)
  assert.match(appSource, /if \(!content \|\| !state\.activeWorkflowRun \|\| workflowAgentSending\.value \|\| !targetScopeId\) return/)
  ...
  const drawerSource = await readFile(new URL('../frontend/src/components/workflow/WorkflowAgentDrawer.vue', import.meta.url), 'utf8')
  const canvasSource = await readFile(new URL('../frontend/src/components/workflow/WorkflowCanvasPage.vue', import.meta.url), 'utf8')
...
testAsync('workflow canvas agent launcher creates a fallback run before opening drawer', async () => {
  const appSource = await readFile(new URL('../frontend/src/App.vue', import.meta.url), 'utf8')
  const helperStart = appSource.indexOf('function ensureActiveWorkflowRunForAgent')
  const helperEnd = appSource.indexOf('function openWorkflowAgentForNode', helperStart)
  const helperSource = appSource.slice(helperStart, helperEnd)
  const openStart = appSource.indexOf('function openWorkflowAgent()')
  const openEnd = appSource.indexOf('function toggleWorkflowAgentHistory', openStart)
  const openSource = appSource.slice(openStart, openEnd)

  assert.ok(helperStart > -1)
  assert.match(helperSource, /if \(state\.activeWorkflowRun\) return state\.activeWorkflowRun/)
  assert.match(helperSource, /workflowAnalysisResult\.value/)
  assert.match(helperSource, /documentAnalysis:\s*workflowAnalysisResult\.value/)
  assert.match(helperSource, /state\.activeWorkflowRun = fallbackRun/)
  assert.match(helperSource, /upsertWorkflowRunRecord\(state\.workflowRuns,\s*fallbackRun\)/)
  assert.match(openSource, /const run = ensureActiveWorkflowRunForAgent\(\)/)
  assert.match(openSource, /if \(!run\) \{[\s\S]*setStatus\(skillWorkbenchStatus,\s*'failed'/)
  assert.match(openSource, /workflowAgentNodeId\.value = workflowAgentNodeId\.value \|\| workflowCanvasNodes\.value\?\.\[0\]\?\.id \|\| 'analysis'/)
  assert.match(openSource, /workflowAgentOpen\.value = true/)
  assert.doesNotMatch(openSource, /if \(!state\.activeWorkflowRun\) return/)
})
...
testAsync('backend workflow agent stream route streams readable structured content and persists final content', async () => {
  const backendRoutes = workflowRoutes(undefined, {
    agentProvider: {
  ...
  assert.match(streamed.body, /event: delta/)
  assert.doesNotMatch(streamed.body, /正在整理结构化建议/)
  const deltaEvents = [...streamed.body.matchAll(/event: delta\ndata: (.+)\n\n/g)].map((match) => JSON.parse(match[1]).content)
  assert.match(deltaEvents.join(''), /已整理目标用户、核心价值和成功标准。/)
  const deltaPayload = deltaEvents.join('\n')
  assert.doesNotMatch(deltaPayload, /"proposal"/)
  assert.doesNotMatch(deltaPayload, /"writeableContent"/)
  assert.doesNotMatch(deltaPayload, /"downstreamImpact"/)
  assert.doesNotMatch(deltaPayload, /正在整理结构化建议/)
  const afterDone = streamed.body.match(/event: done\ndata: (.+)\n\n/)
  assert.ok(afterDone)
  const result = JSON.parse(afterDone[1])
...
testAsync('backend workflow agent stream route waits for readable content after early structured json prefix deltas', async () => {
  const backendRoutes = workflowRoutes(undefined, {
    agentProvider: {
      name: 'fragmented-json-stream-provider',
      async *stream(context) {
        yield { type: 'delta', content: '{"' }
        yield { type: 'delta', content: 'content' }
        yield { type: 'delta', content: '":"已整理风险结论。","proposal":' }
        yield { type: 'delta', content: '{"title":"风险结论补充","summary":"补齐风险指标。"}}' }
        yield {
          type: 'final',
          content: '已整理风险结论。',
          provider: 'fragmented-json-stream-provider',
          model: context.model,
          proposal: {
            title: '风险结论补充',
            summary: '补齐风险指标。'
  ...
  const streamed = await backendRoutes['POST /api/workspace/workflow-runs/:id/messages/stream']({
    ...
    stepId: 'framework',
    model: 'gpt-5.5',
    message: { role: 'user', content: '补风险结论' }
  })

  const deltaEvents = [...streamed.body.matchAll(/event: delta\ndata: (.+)\n\n/g)].map((match) => JSON.parse(match[1]).content)
  assert.deepEqual(deltaEvents, ['已整理风险结论。'])
  const afterDone = streamed.body.match(/event: done\ndata: (.+)\n\n/)
  assert.ok(afterDone)
  const result = JSON.parse(afterDone[1])
  assert.equal(result.assistantMessage.content, '已整理风险结论。')
  assert.equal(result.proposal.title, '风险结论补充')
})
...
testAsync('workflow agent send uses optimistic message and pending assistant feedback', async () => {
  ...
  assert.match(sendSource, /const currentPendingMessage = workflowAgentMessageById\(pendingMessageId,\s*targetScopeId\)/)
  ...
  assert.match(appSource, /后端没有返回 Agent 结果/)
  assert.match(appSource, /后端响应缺少 assistantMessage/)
  assert.match(sendSource, /workflowAgentFailureView\(persisted\)/)
  assert.match(sendSource, /failureView\.content/)
  assert.match(sendSource, /failureView\.recoveryActions/)
  assert.match(appSource, /function isWorkflowAgentNetworkFailure\(result = \{\}\)/)
  assert.match(appSource, /Failed to fetch/)
  assert.match(appSource, /前端没有连到后端 API 服务/)
  assert.match(appSource, /确认 `npm run dev:all` 正在运行/)
...
testAsync('workflow agent app filters malformed references before drawer and backend payloads', async () => {
  ...
  assert.match(persistSource, /references: workflowAgentReferencePayload\(scopeId\)/)
  assert.match(streamSource, /references: workflowAgentReferencePayload\(scopeId\)/)
  assert.match(appSource, /function workflowAgentReferencePayload\(scopeId = workflowAgentScopeId\(\)\)/)
  assert.match(appSource, /return ensureWorkflowReferenceFiles\(scopeId\)\.filter\(isWorkflowAgentReferenceFile\)/)
...
testAsync('workflow agent drawer keeps quick replies visually connected to the composer', async () => {
  const drawerSource = await readFile(new URL('../frontend/src/components/workflow/WorkflowAgentDrawer.vue', import.meta.url), 'utf8')
  const styles = await readFile(new URL('../frontend/src/styles.css', import.meta.url), 'utf8')
  const quickRowStyleStart = styles.indexOf('.agent-quick-row {')
  const quickRowStyleEnd = styles.indexOf('.agent-quick-row button', quickRowStyleStart)
  const quickRowStyle = styles.slice(quickRowStyleStart, quickRowStyleEnd)
  const chatRegionStyleStart = styles.indexOf('.agent-chat-region {')
  const chatRegionStyleEnd = styles.indexOf('.agent-chat-list {', chatRegionStyleStart)
  const chatRegionStyle = styles.slice(chatRegionStyleStart, chatRegionStyleEnd)
  const composerShellStyleStart = styles.indexOf('.agent-composer-shell {')
  const composerShellStyleEnd = styles.indexOf('.agent-editing-banner', composerShellStyleStart)
  const composerShellStyle = styles.slice(composerShellStyleStart, composerShellStyleEnd)

  assert.doesNotMatch(drawerSource, /class="agent-disclaimer"/)
  assert.doesNotMatch(drawerSource, /AI 生成内容可能有误/)
  assert.match(quickRowStyle, /border-top:\s*0/)
  assert.doesNotMatch(quickRowStyle, /border-top:\s*1px/)
  assert.match(quickRowStyle, /border-bottom:\s*0/)
  assert.doesNotMatch(quickRowStyle, /border-bottom:\s*1px/)
  assert.match(chatRegionStyle, /border-bottom:\s*0/)
  assert.doesNotMatch(chatRegionStyle, /border-bottom:\s*1px/)
  assert.match(chatRegionStyle, /border-top:\s*0/)
  assert.doesNotMatch(chatRegionStyle, /border-top:\s*1px/)
  assert.match(composerShellStyle, /border-top:\s*0/)
  assert.doesNotMatch(composerShellStyle, /border-top:\s*1px/)
  assert.match(composerShellStyle, /border-bottom:\s*0/)
  assert.doesNotMatch(composerShellStyle, /border-bottom:\s*1px/)
})
...
testAsync('workflow agent confirmation preview can hand off into fullscreen canvas editing with fixed footer layout', async () => {
  ...
  assert.match(canvasPageSource, /fullscreenEditNodeId:\s*\{\s*type:\s*String,\s*default:\s*''\s*\}/)
...
testAsync('workflow agent locks message writes to the send-time scope so replies do not disappear after node changes', async () => {
  const appSource = await readFile(new URL('../frontend/src/App.vue', import.meta.url), 'utf8')
  const sendStart = appSource.indexOf('async function sendWorkflowAgentMessage')
  const sendEnd = appSource.indexOf('function useWorkflowAgentQuickReply', sendStart)
  const sendSource = appSource.slice(sendStart, sendEnd)
  ...
  const persistEnd = appSource.indexOf('function openWorkflowAgent', persistStart)
  const persistSource = appSource.slice(persistStart, persistEnd)
  const helpersStart = appSource.indexOf('function ensureWorkflowAgentSession')
  const helpersEnd = appSource.indexOf('function workflowAgentMessageById', helpersStart)
  ...
  assert.match(helpersSource, /function ensureWorkflowAgentSession\(scopeId = workflowAgentScopeId\(\)\)/)
  assert.match(helpersSource, /function appendWorkflowAgentMessage\(role,\s*content,\s*options\s*=\s*\{\}\)/)
  assert.match(helpersSource, /const session = ensureWorkflowAgentSession\(options\.scopeId\)/)
  assert.match(helpersSource, /function replaceWorkflowAgentMessage\(messageId,\s*nextMessage,\s*scopeId = workflowAgentScopeId\(\)\)/)
  assert.match(sendSource, /const targetScopeId = options\.nodeId \|\| workflowAgentScopeId\(\)/)
  assert.match(sendSource, /if \(!content \|\| !state\.activeWorkflowRun \|\| workflowAgentSending\.value \|\| !targetScopeId\) return/)
  assert.match(sendSource, /appendWorkflowAgentMessage\('user',\s*content,\s*\{[\s\S]*scopeId:\s*targetScopeId/)
  assert.match(sendSource, /appendWorkflowAgentMessage\('assistant',\s*'正在生成 Agent 回复\.\.\.',\s*\{[\s\S]*scopeId:\s*targetScopeId/)
  assert.match(sendSource, /replaceWorkflowAgentMessage\(pendingMessageId,\s*\{[\s\S]*\},\s*targetScopeId\)/)
  assert.match(sendSource, /retrieveWorkflowKnowledge\(content,\s*targetScopeId\)/)
  assert.match(sendSource, /persistWorkflowAgentMessageStream\(\{[\s\S]*\},\s*pendingStreamOptions\)/)
  assert.match(sendSource, /scopeId:\s*targetScopeId/)
  assert.match(sendSource, /workflowAgentMessageById\(pendingMessageId,\s*targetScopeId\)/)
  assert.match(sendSource, /typeWorkflowAgentAssistantMessage\(pendingMessageId,\s*assistantMessage,\s*\{\s*requestToken,\s*scopeId:\s*targetScopeId\s*\}/)
  assert.match(persistSource, /const scopeId = options\.scopeId \|\| workflowAgentScopeId\(\)/)
  assert.match(persistSource, /replaceWorkflowAgentMessage\(options\.pendingMessageId,[\s\S]*,\s*scopeId\)/)
  assert.match(stopSource, /const pendingScopeId = workflowAgentPendingScopeId\.value \|\| workflowAgentScopeId\(\)/)
  assert.match(stopSource, /replaceWorkflowAgentMessage\(workflowAgentPendingMessageId\.value,[\s\S]*,\s*pendingScopeId\)/)
})
...
testAsync('workflow agent stop generation cancels backend with the pending message client id', async () => {
  ...
  assert.match(stopSource, /const pendingScopeId = workflowAgentPendingScopeId\.value \|\| workflowAgentScopeId\(\)/)
  assert.match(stopSource, /const pendingMessage = workflowAgentPendingMessageId\.value[\s\S]*ensureWorkflowAgentSession\(pendingScopeId\)\.find/)
...
testAsync('workflow agent stream provider failures fallback to non streaming message request', async () => {
  const apiSource = await readFile(new URL('../frontend/src/services/api.js', import.meta.url), 'utf8')
  const streamStart = apiSource.indexOf('appendMessageStream(config, runId, stepId, payload')
  const streamEnd = apiSource.indexOf('cancelMessage(config', streamStart)
  const streamSource = apiSource.slice(streamStart, streamEnd)
  ...
  assert.match(streamSource, /if \(streamed\.status === 'cancelled'\) return streamed/)
  assert.doesNotMatch(streamSource, /if \(streamed\.status === 'failed'\) return streamed/)
  assert.match(streamSource, /if \(streamed\.ok\) return streamed/)
  assert.match(streamSource, /return api\.workflows\.appendMessage\(config,\s*runId,\s*stepId,\s*payload\)/)
})
...
testAsync('workflow agent stream request has timeout and converts timeout into failed card', async () => {
  ...
  assert.match(streamSource, /if \(streamed\.data\?\.timeout\) return streamed/)
...
testAsync('workflow agent keeps stream error failed card after done event without assistant message', async () => {
  ...
  assert.match(sendSource, /if \(streamEventState\.failed && !backendHandled\) \{[\s\S]*workflowAgentPendingMessageId\.value = ''[\s\S]*clearWorkflowAgentActiveDraft\(\)[\s\S]*return/)
  assert.ok(sendSource.indexOf('const backendHandled = Boolean') < sendSource.indexOf('if (streamEventState.failed && !backendHandled)'))
})
...
testAsync('workflow agent stream error keeps failed card and clears active draft', async () => {
  ...
  const sendEnd = appSource.indexOf('function useWorkflowAgentQuickReply', sendStart)
  const sendSource = appSource.slice(sendStart, sendEnd)
  const streamFailedStart = sendSource.indexOf('if (streamEventState.failed && !backendHandled)')
  const streamFailedEnd = sendSource.indexOf('const backendHandled = Boolean', streamFailedStart)
  const nextBackendHandled = sendSource.indexOf('if (backendHandled)', streamFailedStart)
  const streamFailedSource = sendSource.slice(streamFailedStart, nextBackendHandled)
...
testAsync('workflow agent final stream message preserves longer streamed content instead of replacing it with a shorter summary', async () => {
  ...
  assert.match(sendSource, /const currentPendingMessage = workflowAgentMessageById\(pendingMessageId,\s*targetScopeId\)/)
  assert.match(sendSource, /const streamedReplyContent = streamEventState\.failed \? '' : workflowAgentMessageText\(currentPendingMessage\)/)
  assert.match(sendSource, /mergeWorkflowAgentStreamedAssistantMessage\(persisted\.data\.assistantMessage,\s*streamedReplyContent\)/)
...
testAsync('workflow agent streams backend readable deltas while preserving final assistant content', async () => {
  const appSource = await readFile(new URL('../frontend/src/App.vue', import.meta.url), 'utf8')
  const streamStart = appSource.indexOf('async function persistWorkflowAgentMessageStream')
  const streamEnd = appSource.indexOf('function openWorkflowAgent', streamStart)
  const streamSource = appSource.slice(streamStart, streamEnd)
  ...
  assert.match(streamSource, /content:\s*workflowAgentStreamDisplayContent\(streamedContent\)/)
  assert.match(appSource, /return value/)
  assert.doesNotMatch(appSource, /正在整理结构化建议/)
  assert.doesNotMatch(appSource, /if \(isWorkflowAgentStructuredStreamContent\(visibleContent\)\) return normalized/)
  assert.match(sendSource, /mergeWorkflowAgentStreamedAssistantMessage\(persisted\.data\.assistantMessage,\s*streamedReplyContent\)/)
})
...
testAsync('workflow agent messages retrieve project knowledge before backend handoff', async () => {
  const appSource = await readFile(new URL('../frontend/src/App.vue', import.meta.url), 'utf8')
  const drawerSource = await readFile(new URL('../frontend/src/components/workflow/WorkflowAgentDrawer.vue', import.meta.url), 'utf8')
  const agentServiceSource = await readFile(new URL('../backend/services/agent-service.js', import.meta.url), 'utf8')
  const cssSource = await readFile(new URL('../frontend/src/styles.css', import.meta.url), 'utf8')
...
testAsync('workflow agent pending status explains retrieval generation and canvas merge phases', async () => {
  ...
  assert.match(sendSource, /const knowledgeResult = await retrieveWorkflowKnowledge\(content,\s*targetScopeId\)/)
...
testAsync('web factory defaults to image to code and persists restored asset tags', async () => {
  const appSource = await readFile(new URL('../frontend/src/App.vue', import.meta.url), 'utf8')
  const workspaceSource = await readFile(new URL('../frontend/src/services/factoryWorkspace.js', import.meta.url), 'utf8')
  const backendModelSource = await readFile(new URL('../backend/models/workspace.js', import.meta.url), 'utf8')
...
testAsync('backend capture service owns page generation quality gate', async () => {
  const { generatePageFromCapturePayload } = await import('../backend/services/capture-service.js')
  const captureResult = {
    title: 'Screenshot only',
...
testAsync('backend capture runner assembles capture result from injected target capture', async () => {
  const { createCaptureRunner } = await import('../backend/services/capture-runner.js')
  ...
  assert.equal(result.raw.templateId, 'generic-saas-dashboard')
  assert.equal(result.raw.relayMatchReason, 'unit-test-match')
...
testAsync('backend capture runner does not downgrade missing browser auth sessions into public fallback captures', async () => {
  const { createCaptureRunner } = await import('../backend/services/capture-runner.js')
  const runner = createCaptureRunner({
    uuid: () => 'capture-browser-missing-id',
    now: (() => {
      const times = [4000, 5000]
      return () => times.shift() ?? 5000
    })(),
    designRules: {
      colors: {
        primary: '#111111',
    ...
    fallbackCaptureTarget: async () => ({
      ok: true,
      statusCode: 200,
      transport: 'unexpected-fallback',
      html: '<html><body>public fallback should not happen</body></html>'
    })
  })
  ...
  assert.equal(result.taskId, 'capture-browser-missing-id')
  assert.equal(result.status, 'partial')
  assert.equal(result.raw.fetched, false)
  assert.equal(result.raw.transport, null)
  assert.equal(result.raw.fetchStatus, null)
  assert.equal(result.raw.fetchError, '授权浏览器会话不存在或已过期，请重新打开授权浏览器后再采集')
  assert.match(result.pages[0].summary, /授权浏览器会话不存在或已过期/)
  assert.doesNotMatch(result.pages[0].summary, /public fallback should not happen/)
})
...
test('rejects malformed relay URLs with stable relay-owned validation semantics', async () => {
  const { analyzeCaptureUrl } = await import('../backend/services/capture-url-analyzer.js')
  const { createCaptureDispatcher } = await import('../backend/services/capture-dispatcher.js')

  assert.throws(
    () => analyzeCaptureUrl({ url: 'not-a-valid-url', authMode: 'public' }),
    (error) => {
      assert.equal(error.code, 'CAPTURE_URL_INVALID')
      assert.match(error.message, /请输入有效的 HTTP 或 HTTPS 网页 URL/)
      return true
    }
  )
  ...
  const dispatcher = createCaptureDispatcher({
    analyzeCaptureUrl,
    matchCaptureTemplate: () => {
      throw new Error('template matcher should not run for invalid urls')
    },
    compileCaptureTask: () => {
      throw new Error('compiler should not run for invalid urls')
    },
    classifyCaptureDiagnostics: () => ({ status: 'completed', diagnostics: [], recoveryActions: [] }),
    captureRunner: {
      makeCaptureResult: async () => {
        throw new Error('runner should not execute for invalid urls')
      }
    }
  })

  await assert.rejects(
    () => dispatcher.execute({ url: 'ftp://example.com/file.txt', authMode: 'public' }),
    (error) => {
      assert.equal(error.code, 'CAPTURE_URL_INVALID')
      assert.match(error.message, /请输入有效的 HTTP 或 HTTPS 网页 URL/)
      return true
...
testAsync('mock api wires capture start through the relay dispatcher and returns relay metadata', async () => {
  ...
  assert.match(source, /import \{ createCaptureRunner \} from '\.\.\/services\/capture-runner\.js'/)
  assert.match(source, /import \{ createCaptureTaskStore \} from '\.\.\/services\/capture-task-store\.js'/)
...
testAsync('browser authorization uses manual login instead of collecting credentials', async () => {
  const appSource = await readFile(new URL('../frontend/src/App.vue', import.meta.url), 'utf8')
  const serverSource = await readFile(new URL('../backend/server/mock-api.mjs', import.meta.url), 'utf8')
...
testAsync('web factory capture flow preserves backend blocked status in UI state and task history', async () => {
  const appSource = await readFile(new URL('../frontend/src/App.vue', import.meta.url), 'utf8')
  const applyStart = appSource.indexOf('function applyCaptureResult')
  const applyEnd = appSource.indexOf('async function createBrowserSession', applyStart)
  const applySource = appSource.slice(applyStart, applyEnd)
  const runStart = appSource.indexOf('async function runCaptureTask()')
  const runEnd = appSource.indexOf('async function startCapture()', runStart)
  const runSource = appSource.slice(runStart, runEnd)

  assert.match(applySource, /const isBlocked = capture\.status === 'blocked'/)
  assert.match(applySource, /setStatus\(\s*captureStatus,\s*isBlocked \? 'blocked' : 'success'/)
  assert.match(runSource, /const taskStatus = data\.status === 'blocked' \? 'blocked' : 'success'/)
  assert.match(runSource, /message:\s*data\.pages\?\.\[0\]\?\.summary \|\| \(data\.status === 'blocked'/)
  assert.match(runSource, /if \(data\.status === 'blocked'\) \{/)
  assert.match(runSource, /setStatus\(captureStatus,\s*'blocked',\s*data\.pages\?\.\[0\]\?\.summary \|\| '目标网页暂时无法直接采集。'/)
})
...
testAsync('restored page cards open standalone result shell and keep project-owned assets', async () => {
  ...
  assert.match(standaloneSource, /convertUrl:\s*projectScopedUrl\(`\/assets\/\$\{encodeURIComponent\(page\.id \|\| pageId\)\}\/preview\?action=convert-vue`/)
  ...
  assert.match(restoreSource, /state\.currentFactoryRoute\s*=\s*'standalone-preview'/)
  assert.match(restoreSource, /void loadStandalonePreviewRoute\(pageId\)/)
  assert.doesNotMatch(restoreSource, /state\.currentFactoryRoute\s*=\s*'restored-detail'/)
  assert.doesNotMatch(restoreSource, /void openRestoredPageDetail\(pageId\)/)
  assert.match(appSource, /function parseProjectScopedHash[\s\S]*\/\^#\\\/projects\\\/\(\[\^\/\?#\]\+\)\(\\\/\[\^#\]\*\)\?\//)
  assert.match(standaloneSource, /const shouldAutoConvert = new URLSearchParams/)
  assert.match(standaloneSource, /get\('action'\) === 'convert-vue'/)
  assert.match(standaloneSource, /removeProjectRouteQueryParam\('action'\)/)
  assert.match(standaloneSource, /state\.currentFactoryRoute = 'restored-detail'/)
  assert.match(standaloneSource, /await convertRestoredPageToVue\(\)/)
  assert.match(appSource, /v-if="isFactoryView && state\.currentFactoryRoute === 'standalone-preview'"/)
  assert.match(appSource, /<iframe[\s\S]*:key="standalonePreviewKey"[\s\S]*class="standalone-preview-frame"[\s\S]*sandbox="allow-forms allow-scripts allow-top-navigation-by-user-activation"[\s\S]*:srcdoc="standalonePreviewHtml"[\s\S]*><\/iframe>/)
  assert.match(appSource, /const standalonePreviewKey = ref\(0\)/)
  assert.match(appSource, /standalonePreviewKey\.value \+= 1/)
  assert.match(appSource, /state\.currentFactoryRoute === 'standalone-preview'/)
  assert.match(appSource, /buildStaticHtmlResultShell\(html,\s*page\.title/)
  ...
  assert.match(cssSource, /\.standalone-preview-view\s*\{[\s\S]*?position:\s*fixed/)
  assert.match(cssSource, /\.standalone-preview-frame\s*\{[\s\S]*?width:\s*100%/)
})
...
testAsync('web factory capture detail hides the global sidebar', async () => {
  ...
  assert.match(appSource, /const routeHash = currentLocationHash\.value/)
  assert.match(appSource, /const projectRoute = parseProjectScopedHash\(routeHash\)/)
  assert.match(appSource, /const isStandalonePreviewRoute = Boolean\(projectRoute\?\.route\?\.match/)
  assert.match(appSource, /routeHash === '#\/factory\/capture'/)
  assert.match(appSource, /projectRoute\?\.route === '\/design'[\s\S]*return false/)
...
testAsync('app shell declares favicon to avoid browser 404 noise', async () => {
  const htmlSource = await readFile(new URL('../frontend/index.html', import.meta.url), 'utf8')
  const faviconSource = await readFile(new URL('../frontend/public/favicon.svg', import.meta.url), 'utf8')
...
testAsync('web factory opens capture html previews on project scoped routes instead of blank pages', async () => {
  const appSource = await readFile(new URL('../frontend/src/App.vue', import.meta.url), 'utf8')
  const generateStart = appSource.indexOf('async function generatePageFromCapture')
  const generateEnd = appSource.indexOf('function generatedPageBlobUrl')
  const generateSource = appSource.slice(generateStart, generateEnd)

  assert.match(generateSource, /const previewWindow = shouldOpenPreview \? window\.open\(previewTaskUrl\(task\.id\),\s*'_blank'\) : null/)
  assert.doesNotMatch(generateSource, /window\.open\('', '_blank'\)/)
  assert.match(generateSource, /writeStaticHtmlPreviewWindow\(previewWindow,\s*buildStaticHtmlLoadingPage\(/)
  assert.match(generateSource, /writeStaticHtmlPreviewWindow\(previewWindow,\s*buildStaticHtmlLoadingPage[\s\S]*?\)\)\s*\n\s*updateStaticHtmlPreviewUrl\(previewWindow,\s*task\.id\)/)
  assert.match(generateSource, /writeStaticHtmlPreviewWindow\(previewWindow,\s*event\.data\.html\)\s*\n\s*updateStaticHtmlPreviewUrl\(previewWindow,\s*task\.id\)/)
  assert.match(generateSource, /writeStaticHtmlPreviewWindow\(previewWindow,\s*`<!doctype html><html lang="zh-CN"><head><meta charset="UTF-8"><title>采集质量诊断[\s\S]*?\)\s*\n\s*updateStaticHtmlPreviewUrl\(previewWindow,\s*task\.id\)/)
  assert.match(generateSource, /writeStaticHtmlPreviewWindow\(previewWindow,\s*`<!doctype html><html lang="zh-CN"><head><meta charset="UTF-8"><title>资产保存失败[\s\S]*?\)\s*\n\s*updateStaticHtmlPreviewUrl\(previewWindow,\s*task\.id\)/)
})
...
testAsync('web factory capture detail renders backend relay diagnostics and recovery actions', async () => {
  ...
  assert.match(appSource, /const captureRelayTemplateId = computed/)
  assert.match(appSource, /capture-relay-diagnostics/)
  assert.match(appSource, /captureRelayTemplateId \|\| '未命中模板'/)
...
testAsync('backend workspace store recovers corrupt primary file from the best valid backup', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'liuchengtong-workspace-backup-recover-'))
  const filePath = join(dir, 'workspace.json')
  try {
    await writeFile(filePath, '{"projects":[', 'utf8')
    await writeFile(join(dir, 'workspace.json.bak-empty-1.json'), JSON.stringify({
      currentUserId: 'user-local-default',
      currentProjectId: 'project-flow',
      users: [{ id: 'user-local-default', name: '流程通用户' }],
      projects: [
        { id: 'project-flow', ownerUserId: 'user-local-default', name: '流程通默认项目' },
        { id: 'project-podcast', ownerUserId: 'user-local-default', name: 'podcast' },
        { id: 'project-feature-shell', ownerUserId: 'user-local-default', name: '工程开发' }
      ],
      restoredPages: [{ id: 'page-empty-backup', projectId: 'project-flow', title: '低价值旧页' }]
    }), 'utf8')
    await writeFile(join(dir, 'workspace.json.bak-before-project-hierarchy-2.json'), JSON.stringify({
      currentUserId: 'user-local-default',
      currentProjectId: 'project-chanjing',
      users: [{ id: 'user-local-default', name: '流程通用户' }],
      projects: [
        { id: 'project-flow', ownerUserId: 'user-local-default', name: '流程通' },
        { id: 'project-chanjing', ownerUserId: 'user-local-default', name: '蝉镜' }
      ],
      assets: [{ id: 'asset-chanjing', projectId: 'project-chanjing', title: '蝉镜资产' }],
      materials: [{ id: 'knowledge-chanjing', projectId: 'project-chanjing', type: 'knowledge', title: '蝉镜知识' }],
      restoredPages: [{ id: 'page-chanjing', projectId: 'project-chanjing', title: '蝉镜页面', html: '<main>蝉镜</main>' }],
      workflowRuns: [{ id: 'run-flow', projectId: 'project-flow', title: '流程通代码流程' }]
    }), 'utf8')
    ...
    assert.equal(snapshot.currentProjectId, 'project-chanjing')
    assert.deepEqual(snapshot.projects.map((project) => project.name).sort(), ['流程通', '蝉镜'])
    assert.equal(snapshot.restoredPages.find((page) => page.id === 'page-chanjing')?.projectId, 'project-chanjing')
    assert.equal(savedFile.currentProjectId, 'project-chanjing')
    assert.equal(savedFile.projects.length, 2)
    assert.equal(savedFile.assets.find((asset) => asset.id === 'asset-chanjing')?.projectId, 'project-chanjing')
    assert.ok(files.some((file) => /^workspace\.json\.corrupt-/.test(file)))
  } finally {
    await rm(dir, { recursive: true, force: true })
  }
...
testAsync('workflow analysis deep links load full run detail instead of overview summary', async () => {
  ...
  assert.match(hydrateSource, /const routeProjectId = parseProjectScopedHash\(window\.location\.hash\)\?\.projectId/)
...
testAsync('model settings provider applies insecure tls wrapper without explicit fetch override', async () => {
  const previousFetch = globalThis.fetch
  const previousTls = process.env.NODE_TLS_REJECT_UNAUTHORIZED
  let observedTls = ''
  globalThis.fetch = async () => {
    observedTls = process.env.NODE_TLS_REJECT_UNAUTHORIZED || ''
    return {
      ok: true,
      async json() {
        return {
  ...
  try {
    ...
    await provider.generate({
      systemPrompt: '测试 TLS 配置',
      userPrompt: 'ping'
    })

    assert.equal(observedTls, '0')
    assert.equal(process.env.NODE_TLS_REJECT_UNAUTHORIZED, previousTls)
  } finally {
...
test('openai compatible provider wires stream signal into sse body reader', () => {
  const providerSource = readFileSync(new URL('../backend/services/llm-provider.js', import.meta.url), 'utf8')
...
test('Podcastor delivery output separates frontend and backend files', () => {
  ...
  assert.match(output, /frontend\/src\/App.vue/)
...
testAsync('workflow analysis deep links survive backend hydration and stale async detail loads', async () => {
  const appSource = await readFile(new URL('../frontend/src/App.vue', import.meta.url), 'utf8')
  const routeStart = appSource.indexOf('function restoreAppRouteFromUrl')
  const routeEnd = appSource.indexOf('function restoreProjectScopedRoute', routeStart)
  const routeSource = appSource.slice(routeStart, routeEnd)
  const ensureStart = appSource.indexOf('function ensureWorkflowDeepLinkRouteState')
  const ensureEnd = appSource.indexOf('function stopWorkflowAnalysisDeepLinkPolling', ensureStart)
  ...
  const restoreEnd = appSource.indexOf('function returnToWorkflowEntry', restoreStart)
  const restoreSource = appSource.slice(restoreStart, restoreEnd)
  const openRunStart = appSource.indexOf('function openWorkflowCanvasRun')
  const openRunEnd = appSource.indexOf('function openWorkflowAnalysisRecord', openRunStart)
  const openRunSource = appSource.slice(openRunStart, openRunEnd)

  assert.ok(routeStart > 0)
  assert.ok(ensureStart > 0)
  assert.ok(hydrateStart > 0)
  assert.ok(restoreStart > 0)
  assert.match(appSource, /function currentWorkflowAnalysisDeepLinkRunId\(/)
  assert.match(appSource, /function restoreWorkflowDeepLinkFromCurrentHashSoon\(/)
  assert.match(appSource, /function ensureWorkflowDeepLinkRouteState\(/)
  assert.match(routeSource, /if \(currentWorkflowAnalysisDeepLinkRunId\(hash\)\) \{[\s\S]*ensureWorkflowDeepLinkRouteState\(\)[\s\S]*return restoreWorkflowAnalysisFromUrl\(parseProjectScopedHash\(hash\)\)[\s\S]*\}/)
  assert.match(ensureSource, /activeView\.value = 'workflow'/)
  assert.match(ensureSource, /workflowRoute\.value = 'canvas'/)
  assert.match(appSource, /restoreWorkflowDeepLinkFromCurrentHashSoon\(\)/)
  assert.match(appSource, /window\.setTimeout\(\(\) => \{[\s\S]*ensureWorkflowDeepLinkRouteState\(\)[\s\S]*restoreWorkflowAnalysisFromUrl\(parseProjectScopedHash\(window\.location\.hash\)\)[\s\S]*\},\s*0\)/)
  assert.match(hydrateSource, /const workflowDeepLinkRunId = currentWorkflowAnalysisDeepLinkRunId\(\)/)
  assert.match(hydrateSource, /if \(workflowDeepLinkRunId\) \{[\s\S]*restoreWorkflowAnalysisFromUrl\(parseProjectScopedHash\(window\.location\.hash\)\)[\s\S]*restoreWorkflowDeepLinkFromCurrentHashSoon\(\)[\s\S]*return[\s\S]*\}/)
  assert.match(restoreSource, /ensureWorkflowDeepLinkRouteState\(\)/)
  assert.match(restoreSource, /if \(currentWorkflowAnalysisDeepLinkRunId\(\) !== runId\) return/)
  assert.match(restoreSource, /loadWorkflowRunDetail\(runId,\s*\{ fallbackRun:\s*run \}\)\.then/)
  assert.match(openRunSource, /if \(currentWorkflowAnalysisDeepLinkRunId\(\) === run\.id\) syncWorkflowAnalysisRoute\(run\.id,\s*'replace'\)/)
})

testAsync('workflow analysis deep links block requirements fallback while detail loads', async () => {
  const appSource = await readFile(new URL('../frontend/src/App.vue', import.meta.url), 'utf8')
  const routeStart = appSource.indexOf('function applyRouteState')
  const routeEnd = appSource.indexOf('function switchView', routeStart)
  const routeSource = appSource.slice(routeStart, routeEnd)
  ...
  assert.ok(routeStart > 0)
  assert.match(appSource, /function isWorkflowAnalysisRouteActive\(/)
  assert.match(routeSource, /if \(key !== 'workflow' && isWorkflowAnalysisRouteActive\(\)\) \{[\s\S]*ensureWorkflowDeepLinkRouteState\(\)[\s\S]*return[\s\S]*\}/)
  assert.ok(routeSource.indexOf('isWorkflowAnalysisRouteActive()') < routeSource.indexOf("if (key === 'requirements')"))
})
...
testAsync('backend model settings can persist insecure tls flag without exposing it to frontend', async () => {
  const store = createWorkspaceStore()
  const backendRoutes = workspaceRoutes(store)
  ...
  const rawSettings = store.settings.find((item) => item.key === 'workflowModelProvider').value

  assert.equal(rawSettings.allowInsecureTLS, true)
  assert.equal(saved.modelSettings.allowInsecureTLS, undefined)
})
...
testAsync('workflow canvas frontend displays analysis version and quality gate metadata', async () => {
  const componentSource = await readFile(new URL('../frontend/src/components/workflow/WorkflowCanvasPage.vue', import.meta.url), 'utf8')
  const appSource = await readFile(new URL('../frontend/src/App.vue', import.meta.url), 'utf8')
...
testAsync('backend analysis quality service compares analysis version snapshots', async () => {
  const { buildAnalysisVersionDiff } = await import('../backend/services/analysis-quality.js')
...
testAsync('workflow canvas version history displays applied node diff summaries', async () => {
  const componentSource = await readFile(new URL('../frontend/src/components/workflow/WorkflowCanvasPage.vue', import.meta.url), 'utf8')
  const stylesSource = await readFile(new URL('../frontend/src/styles.css', import.meta.url), 'utf8')
...
testAsync('backend quality gate provides repair suggestions for failed checks', async () => {
  const { buildAnalysisQualityGate } = await import('../backend/services/analysis-quality.js')
...
testAsync('backend agent context owns canvas action prompt details from structured payload', async () => {
  const appSource = await readFile(new URL('../frontend/src/App.vue', import.meta.url), 'utf8')
  const contextSource = await readFile(new URL('../backend/services/agent-context-builder.js', import.meta.url), 'utf8')
  const sendStart = appSource.indexOf('async function sendWorkflowAgentMessage')
  const sendEnd = appSource.indexOf('function useWorkflowAgentQuickReply', sendStart)
  const sendSource = appSource.slice(sendStart, sendEnd)
...
testAsync('workflow canvas quick action pins request context active node to clicked canvas node', async () => {
  ...
  assert.match(persistStreamSource, /nodeId:\s*options\.nodeId \|\| scopeId/)

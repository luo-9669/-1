...
test('competitor framework detail uses table explorer with expandable rows instead of card grids', async () => {
  const pageSource = await readFile(pageUrl, 'utf8')
  const frameworkStart = pageSource.indexOf('function frameworkRowTypeForBlock')
  const frameworkEnd = pageSource.indexOf('function markdownBlockKey', frameworkStart)
  const frameworkSource = pageSource.slice(frameworkStart, frameworkEnd)
  ...
  assert.match(frameworkSource, /frameworkRowFromMarkdownTable/)
  assert.match(frameworkSource, /功能模块/)
  assert.match(frameworkSource, /页面清单/)
  assert.match(frameworkSource, /入口路径/)
  assert.match(pageSource, /selectedRecord\?\.kind === 'framework' && selectedFrameworkRows\.length/)
...
test('competitor analysis detail footer exposes report actions', async () => {
  ...
  assert.doesNotMatch(pageSource, /selectedDetailMetaRows/)
  assert.doesNotMatch(pageSource, /competitor-analysis-detail-meta/)
  assert.doesNotMatch(pageSource, /detailMetaRowsForRecord/)
  assert.doesNotMatch(pageSource, /formatDuration/)
...
test('competitor analysis report detail omits the summary meta strip above report content', async () => {
  const pageSource = await readFile(pageUrl, 'utf8')

  assert.doesNotMatch(pageSource, /aria-label="报告信息"/)
  assert.doesNotMatch(pageSource, /class="competitor-analysis-detail-meta"/)
  assert.doesNotMatch(pageSource, /selectedDetailMetaRows/)
  assert.doesNotMatch(pageSource, /function detailMetaRowsForRecord/)
  assert.doesNotMatch(pageSource, /function recordEvidenceQuality/)
})

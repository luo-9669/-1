...
Temporary image-to-HTML preview routes must keep waiting for backend persistence without a frontend maximum timeout. For `image-html-*` ids, the standalone preview page polls every 15 seconds, keeps the loading shell stable between polls, and uses a route/load token such as `standalonePreviewLoadToken` so stale refresh or hydration loops cannot overwrite the active iframe or cause flicker. Do not reintroduce `TEMPORARY_IMAGE_HTML_PREVIEW_MAX_WAIT_MS` or a short 1-2 second polling loop for these temporary preview ids.
...
Generated UI images and generated HTML should use fixed logical widths, not fixed heights: Web/desktop targets use a 1920px logical width, mobile/H5 targets use a 375px logical width, and page height must grow naturally with content. Prompts and generated CSS must not stretch, squeeze, or crop UI screenshots to force a fixed height; image regions should preserve intrinsic ratios through `aspect-ratio`, `object-fit`, `max-width`, and `max-height`.
...
- Temporary standalone preview ids must poll at the contract interval and remain cancellable by a load token. Multiple route restoration, focus, visibility, or hydration events may start overlapping waits; only the latest wait may write `standalonePreviewHtml` or replace the route.
...
- Image-to-HTML regression tests must also protect that temporary `image-html-*` standalone preview waits without a frontend maximum timeout, polls at 15 seconds, and keeps a load token guard to prevent stale waits from overwriting refreshed previews.

...
import { buildVisualVerificationReport } from '../src/services/visualVerification.js'
import { compareImageDataUrls } from '../src/services/visualVerification.node.js'
import { createRestoredPageAsset } from '../src/services/factoryWorkspace.js'
import { createWorkspaceStore, workspaceRoutes } from '../后端/routes/workspace.js'
import { addModelCallLog, addRestoredPage, getModelSettingsRaw, saveModelSettings } from '../后端/services/workspace-store.js'
...
import { generatePageFromCapturePayload } from '../后端/services/capture-service.js'
import { createCaptureRunner } from '../后端/services/capture-runner.js'
import { createCaptureTaskStore } from '../后端/services/capture-task-store.js'
import { createAgentProviderFromEnv, createAgentProviderFromModelSettings } from '../后端/services/llm-provider.js'
...
const SINGLE_FILE_BIN = resolve(projectRoot, 'node_modules/.bin/single-file')
...
const workspaceStore = createWorkspaceStore(null, {
  filePath: isTestRuntime
    ? ''
    : (process.env.WORKSPACE_STORE_FILE || resolve(fileURLToPath(new URL('..', import.meta.url)), 'storage/workspace/workspace.local.json'))
})
await workspaceStore.load()
async function ensureCcswitchModelSettings(store) {
  if (isTestRuntime) return
  const settings = getModelSettingsRaw(store)
  const alreadyCcswitch = settings?.enabled &&
    settings?.provider === CODEX_CCSWITCH_MODEL_SETTINGS.provider &&
    settings?.apiKey === CODEX_CCSWITCH_MODEL_SETTINGS.apiKey &&
    settings?.baseUrl === CODEX_CCSWITCH_MODEL_SETTINGS.baseUrl &&
    settings?.defaultModel === CODEX_CCSWITCH_MODEL_SETTINGS.defaultModel &&
    settings?.apiSurface === CODEX_CCSWITCH_MODEL_SETTINGS.apiSurface &&
    Number(settings?.timeoutMs) === CODEX_CCSWITCH_MODEL_SETTINGS.timeoutMs
  if (alreadyCcswitch) return
  await saveModelSettings(store, CODEX_CCSWITCH_MODEL_SETTINGS)
}
...
function createDefaultWorkflowAgentProvider(fetchImpl = globalThis.fetch) {
  ...
  return createAgentProviderFromModelSettings(CODEX_CCSWITCH_MODEL_SETTINGS, fetchImpl)
}

await ensureCcswitchModelSettings(workspaceStore)
...
function resolveImageGenerationAgentProvider(options = {}) {
  ...
  if (settings?.enabled) return createAgentProviderFromModelSettings(settings, options.fetchImpl || globalThis.fetch)
...
async function designMarkdown() {
  ...
  try {
    cachedDesignMarkdown = await readFile(new URL('../design.md', import.meta.url), 'utf8')
  } catch {
...
function imageToStructuredHtml(payload = {}) {
  ...
  const safePrompt = escapeHtml(prompt || '根据上传设计图生成可编辑的静态页面结构')
  const sectionCards = (visualModel.sections || []).map((section) => `
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    .screenshot-restore-page { min-height: 100vh; display: grid; grid-template-columns: 248px minmax(0, 1fr); background: #f6f7f9; }
    ...
    .menu span { border-radius: 8px; padding: 11px 12px; color: #606873; }
    .menu .active { background: #222529; color: #fff; }
    .restore-workspace { padding: 28px 36px 48px; }
    .restore-topbar, .restore-tabs, .chart-card, .table-card, .analysis-card, .json-card { background: #fff; border: 1px solid #e8eaec; border-radius: 12px; }
    .restore-topbar { min-height: 76px; display: flex; align-items: center; justify-content: space-between; gap: 18px; padding: 0 22px; }
    h1 { margin: 0 0 6px; font-size: 28px; letter-spacing: 0; }
    .lead { margin: 0; color: #7f8792; font-size: 14px; line-height: 1.7; }
    .actions { display: flex; gap: 10px; }
    button { height: 36px; border: 1px solid #dfe3e8; border-radius: 8px; background: #fff; padding: 0 14px; color: #222529; font-weight: 700; }
    button.primary { background: #222529; color: #fff; border-color: #222529; }
    .restore-tabs { display: flex; gap: 8px; padding: 8px; margin: 18px 0; overflow-x: auto; }
    .restore-tabs button.active { background: #eefdff; border-color: #69e1f5; }
    .restore-grid { display: grid; grid-template-columns: minmax(0, 1.25fr) minmax(360px, .75fr); gap: 18px; }
    .chart-card, .table-card, .json-card { padding: 20px; min-width: 0; }
    .chart-card h2, .table-card h2, .json-card h2 { margin: 0 0 14px; font-size: 18px; }
    .mermaid { margin: 0; min-height: 250px; border-radius: 10px; background: #f8fafc; padding: 16px; color: #475569; overflow: auto; }
    table { width: 100%; border-collapse: collapse; font-size: 13px; }
    th, td { border-bottom: 1px solid #edf0f2; padding: 10px 8px; text-align: left; }
    th { color: #7f8792; font-weight: 700; }
    .analysis-cards { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 14px; margin-top: 18px; }
    .analysis-card { padding: 16px; }
    .analysis-card strong { display: block; margin-bottom: 8px; }
    .analysis-card p { margin: 0; color: #606873; font-size: 13px; line-height: 1.7; }
    .json-card { margin-top: 18px; }
    .json-card pre { margin: 0; max-height: 220px; overflow: auto; color: #475569; background: #f8fafc; border-radius: 10px; padding: 14px; }
    @media (max-width: 900px) {
      .screenshot-restore-page { grid-template-columns: 1fr; }
      .restore-sidebar { border-right: 0; border-bottom: 1px solid #e8eaec; }
      .restore-grid, .analysis-cards { grid-template-columns: 1fr; }
      .restore-topbar { align-items: flex-start; flex-direction: column; padding: 18px; }
    }
  </style>
</head>
<body>
  <main class="screenshot-restore-page" data-restore-source="screenshot">
    <aside class="restore-sidebar">
      <div class="brand">${safeTitle}</div>
      <nav class="menu" aria-label="截图还原导航">
        <span class="active">视觉结构解析</span>
        <span>工作流分析</span>
    ...
    <section class="restore-workspace">
      <header class="restore-topbar">
        <div>
          <h1>截图还原 HTML</h1>
          <p class="lead">${safePrompt}</p>
        </div>
        <div class="actions">
          <button type="button" data-copy>复制 HTML</button>
          <button class="primary" type="button">下载 MD</button>
        </div>
      </header>
      <nav class="restore-tabs" aria-label="分析 Tab">
        <button class="active" data-tab="overview">项目概览</button>
        <button data-tab="journey">用户旅程</button>
        <button data-tab="tree">机会树</button>
        <button data-tab="priority">优先级打分</button>
        <button data-tab="spec">页面设计规范</button>
        <button data-tab="review">设计评审</button>
      </nav>
  ...
  section 评审输出
    ...
    导出设计说明: 5: 团队</pre>
        ...
        <article class="table-card">
          <h2>RICE 优先级表</h2>
          <table>
            <thead><tr><th>模块</th><th>Reach</th><th>Impact</th><th>Confidence</th><th>Effort</th></tr></thead>
            <tbody>
              <tr><td>用户旅程图</td><td>80</td><td>3</td><td>90%</td><td>2</td></tr>
              <tr><td>机会方案树</td><td>64</td><td>4</td><td>85%</td><td>3</td></tr>
              <tr><td>设计评审清单</td><td>52</td><td>3</td><td>80%</td><td>2</td></tr>
            </tbody>
          </table>
        </article>
      </section>
      ...
      <section class="json-card">
        <h2>结构化视觉模型</h2>
        <pre>${escapeHtml(JSON.stringify(visualModel, null, 2))}</pre>
      </section>
    </section>
  </main>
  <script>
...
async function generateImageVisionModel(payload = {}, options = {}) {
  const provider = resolveImageGenerationAgentProvider(options)
...
async function generateImageToHtml(payload = {}, options = {}) {
  ...
  try {
    modelGeneration = await generateImageVisionModel(payload, options)
  } catch (error) {
    fallbackReason = error.message || '视觉模型调用失败'
  }
  ...
  const html = modelGeneration?.html || imageToStructuredHtml({ ...payload, visualModel })
  const visualVerification = {
    url: captureResult.url,
    status: 'passed',
    thresholds: { mode: modelGeneration ? 'vision-model' : 'deterministic-mock' },
    summary: { total: 1, passed: 1, failed: 0, averageScore: 100 },
    results: [
      {
        id: 'desktop',
        width: 1440,
    ...
    recommendations: ['本地 mock 已生成结构化 HTML；生产环境应接入视觉模型和截图对照验收。']
  }
...
async function generateImageToHtmlStream(payload = {}, options = {}, routeContext = {}) {
  ...
  if (result.fallbackUsed) {
    stream.push('status', {
      status: 'fallback',
      label: `视觉模型暂未返回，已使用本地结构化方案继续生成${result.fallbackReason ? `：${result.fallbackReason}` : ''}`
    })
  }
